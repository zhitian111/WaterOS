...
global_asm!(include_str!("../../arch/loongarch/trap.asm"));
...
fn handle_syscall(tf: &mut TrapFrame) {
    ...
    let args = [tf.regs[4], tf.regs[5], tf.regs[6], tf.regs[7], tf.regs[8], tf.regs[9], tf.regs[10]];
...
pub extern "C" fn rust_main() -> ! {
    ...
    let user_stack = unsafe { USER_STACK.as_ptr() as usize + USER_STACK_SIZE };
