...
脚本实现位于 `os/scripts/`，从 `os/` 目录运行。优先使用 `make` 目标；直接运行脚本前
先检查其参数和是否会改写镜像、配置或工作树。Git 会保存已跟踪脚本的可执行权限，无需
批量 `chmod`。

完整工具入口见 [`README.md`](./README.md)。本页只记录脚本与 Makefile 未覆盖功能的对应关系。
...
GDB/LLDB 的完整操作流程见
[`docs/tools/debugging.md`](../debugging.md)。
