...
- [x] `kernel_main` 在 riscv64 路径上存在**单一、可读**的「bring-up 总线」注释块，顺序与 `main.rs` 文档一致。
- [x] 每个已合并的 bring-up 阶段在失败时行为明确：**`warn` 后继续** vs **`panic`/停机**；默认与 `fs::test` 一致为 **非致命 warn**（除非某阶段契约定义为硬门禁）。
- [x] 不通过 `self_tests` 即可在日志中看到各 `stage` 的 **BEGIN/END** 或等价成对标记（由各子包实现，总线只负责调用次序）。
