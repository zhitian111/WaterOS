...
%
% 事实来源：os/Makefile、docs/tasks/run_testsuits_qemu.md、os/src/user_bringup_*.rs
...
\begin{longtable}{>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{4.8cm}>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{3.8cm}}
  \caption{赛题测例阶段（P1–P6）}\label{tab:test-phases}\\
  \toprule[1.5pt]
  阶段 & 脚本（glibc/musl各一份） & 主要覆盖 & 环境注意 \\
  \midrule[1pt]
...
阶段批跑可调用\code{os/scripts/run\_phase\_tests.sh}（会临时改写\code{BRINGUP\_COMMANDS}并在结束后恢复）。单次手动回归示例：
...
\begin{longtable}{>{\raggedright\arraybackslash}p{3.0cm}>{\raggedright\arraybackslash}p{5.0cm}>{\raggedright\arraybackslash}p{5.2cm}}
  ...
  \midrule[1pt]
  构建产物 & \code{make kernel-rv}→\code{kernel-rv}（\code{riscv64gc-unknown-none-elf}）。 & \code{make kernel-la}→\code{kernel-la}（\code{loongarch64-unknown-none}）。 \\
...
测例覆盖面扩大后，失败往往落在syscall语义、挂载路径、资源回收或双架构底层差异上。下面按主题说明典型现象、处理思路，并给出当前代码中的关键片段（路径均相对于\code{os/}）。

\subsection{系统调用语义和错误码}

\textbf{现象。}libc与LTP会校验\syscall{stat}/\syscall{fstat}/\syscall{statx}字段、\syscall{mount}标志以及别名syscall（如\code{faccessat2}）。占位返回值或静默忽略unsupported flags会在更深路径上变成错误语义；号表未收录的syscall在bring-up期会走\code{unsupported: unknown nr=}直接panic。
...
别名syscall未实现时统一\code{ENOSYS}（\code{syscall-impl/impl-kernel/src/lib.rs}）：
...
bring-up总线入口（\code{src/user\_bringup\_bus.rs}）：
...
调度器中断守卫（\code{wateros-task/.../impl-round-robin/src/lib.rs}）：
...
脚本结束清理（\code{src/user\_bringup\_common.rs}）：
...
socket fd注册表核心（\code{syscall-impl/impl-kernel/src/socket\_fd.rs}）：
...
\textbf{现象。}QEMU无输出、PC卡死、\code{unknown nr=}、用户任务不退出或脚本\code{exit\_code}非零。

\textbf{处理。}按层读串口日志：平台与\code{[self-test]}→驱动\code{[driver*]}→FS\code{[fs*]}→\code{[bringup]}→\code{[busybox-bringup]}→测例\code{COMP TEST GROUP}。需要指令级定位时用\code{make rv\_pc\_watch}/\code{la\_pc\_watch}只看PC跳变，用\code{rv\_symbol\_at ADDR=…}把地址解析到内核符号；长跑日志用\code{rv\_qemu\_run\_with\_log}生成\code{qemu.log}配合\code{grep}，测例摘要仍用\code{parse\_qemu\_test\_log.py}。

典型检索命令：

\begin{bashcode}
LOG=/tmp/wateros_P1.log   # 或 os/log、la_local_run_*.log

# 内核 panic 与未知 syscall
grep -nE 'unsupported: unknown nr=|Panicked at' "$LOG"

# 用户态测例失败
grep -nE 'testcase busybox .+ fail|Assert Fatal' "$LOG"

# syscall 返回负 errno（ret=-N）
grep -nE '\[syscall\] nr=[0-9]+ ret=-[0-9]+' "$LOG"

# 将 PC 解析到内核符号（在 os/ 目录下执行）
make la_symbol_at ADDR=0x90001234
\end{bashcode}

建议保存完整日志到\code{/tmp/wateros\_P*.log}或\code{os/log}（勿提交git），记录当次\code{BRINGUP\_COMMANDS}注释状态，便于与双架构对比结论一并归档。
