...
\begin{itemize}
  \item \code{addr}：\code{BasePhysAddr}、\code{BaseVirtAddr}、\code{BasePPN}、\code{BaseVPN}（\code{repr(C)}，\code{val: usize}）。\code{BasePhysAddr}/\code{BaseVirtAddr}实现\code{Into<*mut T>}薄转换，不校验对齐或映射有效性；
...
\code{fs}：页缓存\code{FILE\_PAGE\_SIZE = 4096}、大文件阈值64KiB、全局页缓存4096槽、读预取步长8页、\code{FileIoMode::Direct}（\code{Async}在配置层存在但v1未实现）、块设备LBA缓存1024块。
