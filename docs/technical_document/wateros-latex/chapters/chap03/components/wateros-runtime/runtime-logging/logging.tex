...
\code{wateros-runtime-logging}把\code{log} crate接到\code{runtime-console}：\code{init()}按feature设置\code{LevelFilter}并注册静态\code{WaterOSLogger}；成功后会打印一条Info级Logger initialized确认。\code{trace!}/\code{debug!}/\code{info!}/\code{warn!}/\code{error!}重导出\code{log}宏，语义与上游一致。
