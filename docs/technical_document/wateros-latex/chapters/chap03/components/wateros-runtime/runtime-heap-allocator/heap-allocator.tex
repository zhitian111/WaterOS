...
\code{wateros-runtime-heap-allocator}实现内核全局\code{GlobalAlloc}：默认\code{impl-linked-list-allocator}（\code{linked\_list\_allocator::LockedHeap}），可选\code{impl-tlsf}（\code{rlsf::Tlsf}），二者互斥由\code{compile\_error!}守卫。堆池大小\code{KERNEL\_HEAP\_SIZE}来自\code{wateros-base-config}；静态\code{HEAP\_SPACE}数组可由链接脚本映射到\code{kernel\_heap}符号。
