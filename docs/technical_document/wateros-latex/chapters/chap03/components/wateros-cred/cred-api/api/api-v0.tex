...
快照包含八ID（real/effective/saved/fs的uid与gid）及最多32个supplementary组。\code{ROOT}常量：全部ID为0，\code{supplementary\_group\_len = 1}且组为\code{[0]}。
...
v0对齐Linux cred子集，不含namespace、securebits、完整cap位图。\code{on\_exec}契约保留S\_ISUID/S\_ISGID解析扩展点（\code{TODO(cred-exec-setuid)}），当前impl为no-op。
