...
\code{PlatformTime}仅承诺\code{time\_frequency\_hz()}：返回调度与tick换算采用的Hz。默认实现返回\code{PlatformTimeError::Unsupported}；引导期若已从DTB写入频率，应经聚合层\code{set\_frequency\_hz}缓存后再供\code{timer}使用，避免与arch层\code{read\_time\_frequency}（常不支持）混淆。
...
\code{PlatformReset}映射\code{PlatformResetType}（\code{Shutdown}/\code{ColdReboot}/\code{WarmReboot}）与\code{PlatformResetReason}；\code{platform\_shutdown}/\code{platform\_reboot}在\code{is\_available}为false时直接返回\code{Unavailable}，否则委托\code{platform\_reset}。默认\code{platform\_reset}返回\code{Unsupported}，由\code{platform-impl}覆盖。
