...
早期控制台为16550 MMIO UART（基址\code{0x1FE0\_01E0}）：写\code{THR}前轮询\code{LSR.THRE}，输出换行时先写回车再写换行。定时器\code{set\_timer}读\code{rdtime.d}得当前tick，计算与deadline的\code{delta}后写\code{CSR\_TCFG}（\code{(delta << 2) | ENABLE}）并清\code{CSR\_TICLR}。关机/重启经ACPI GED寄存器\code{0x100e\_001c}写\code{SLEEP\_CTL}或\code{RESET}；写后通常返回\code{PlatformResetError::Failed}并在调用方循环等待QEMU退出。
