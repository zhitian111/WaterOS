...
\paragraph{数据结构。}
全局\code{GlobalCacheState}：固定\code{PageFrame}数组、\code{BTreeMap}索引、LRU队列与空闲列表。每文件 \code{FileEntryInner}，即 \code{Arc<RwLock<...>>}，跟踪逻辑文件长度\code{logical\_size}，可大于底层ext4 size，覆盖未刷回写、脏页集合与detached小文件缓冲。
...
\paragraph{读写与刷回。}
\code{read\_range}/\code{write\_range}按页命中或分配帧；miss经\code{PageCacheIo}下探\code{FsBridge}。\code{logical\_size}在写超EOF时扩展，\code{metadata}查询经\code{overlay\_cached\_size}反映缓存视图。\code{flush\_file}按\code{FLUSH\_RUN\_MAX\_PAGES}批量写回脏页；\code{reset\_file\_page\_cache}，即聚合 \code{vfs::reset\_file\_page\_cache}，全量刷回并清空索引。
