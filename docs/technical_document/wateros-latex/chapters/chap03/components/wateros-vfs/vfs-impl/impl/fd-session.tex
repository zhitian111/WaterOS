...
以\code{task::TaskId}为键维护\code{BTreeMap} fd表：槽位\code{0/1/2}预置stdin/stdout/stderr，控制台句柄，\code{VFS\_FIRST\_DYNAMIC\_FD}，3起动态分配。支持fork/clone共享fd表，经 \code{owners}/\code{ref\_counts} 引用计数；\code{FD\_CLOEXEC} 与 \code{FD\_PATH\_ONLY}，即 \code{O\_PATH}，标志。
...
\paragraph{文件锁。}
\code{Flock}/\code{InodeKey}实现建议性\code{flock}，含 \code{LOCK\_SH}/\code{LOCK\_EX}/\code{LOCK\_NB}/\code{LOCK\_UN}，按inode键全局串行化冲突检测。
