...
虚拟文件系统叠在文件系统组件与系统调用之间，把路径上的操作和每个进程手里的 fd分开处理。它负责路径规范化、挂载表路由、普通文件的页缓存，以及每任务的工作目录和 fd 表；管道端点和部分字符设备也在这里落成可 read/write 的句柄。块格式与 ext4 细节不在这个组件里，由桥接层转调文件系统组件。
