...
\paragraph{设备类型与MMIO摘要。}\code{DeviceType}区分\code{Block}、\code{Character}、\code{Network}与\code{Unknown}；VirtIO-MMIO魔数\code{0x74726976}配合\code{device\_id}，2=块，1=网在平台impl中填充。\code{MmioRegion}承载DTB\code{reg}首段\code{\{base, size\}}；\code{size==0}的条目由调用方丢弃。\code{IrqLine}解析\code{interrupts}首字与可选\code{interrupt-parent}；复杂PLIC复用形态当前返回\code{None}而非误解析。
