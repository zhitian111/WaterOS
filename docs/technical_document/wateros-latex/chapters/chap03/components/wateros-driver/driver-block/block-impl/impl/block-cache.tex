...
\code{BlockCacheManager::default\_config}读取\code{base-config}容量；\code{wrap}返回可直接\code{register\_block\_device}的\code{SharedBlockDevice}。\code{flush\_all}当前恒\code{Ok(())}。单元测试覆盖「同块重复读仅一次底层读」「连续未命中合并读」「写后读命中缓存」等路径。
