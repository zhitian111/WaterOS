...
\paragraph{stack模块。}\code{init(ip, gateway)}用首张网卡或回环模式构造\code{Interface}，配置\code{/24}本机地址、\code{127.0.0.0/8}与默认路由。\code{poll\_at\_millis}以调用方单调毫秒驱动\code{iface.poll}。\code{SocketMeta}在内核侧跟踪\code{SocketState}，含 \code{Created}/\code{Bound}/\code{Listening}/\code{Connecting}/\code{Connected}/\code{Closed}，ephemeral端口、\code{TCP\_NODELAY}、\code{SO\_RCVTIMEO}记录值与组播成员集合。

\paragraph{socket工厂与操作。}\code{create\_tcp\_socket}/\code{create\_udp\_socket}向\code{SocketSet}添加smoltcp socket并插入元数据。\code{socket\_bind}对TCP预分配ephemeral端口，满足 netperf 在 \code{listen} 前 \code{getsockname}；UDP调用smoltcp\code{bind}。\code{socket\_connect}、\code{socket\_listen}/\code{accept}、\code{send}/\code{recv}、\code{sendto}/\code{recvfrom}覆盖INET测程主路径。\code{poll\_socket\_events}将\code{Connecting}同步为\code{Connected}。
...
\paragraph{VFS桥。}\code{socket\_handles}，同feature将\code{SocketHandle}包装为\code{TcpStreamHandle}/\code{UdpSocketHandle}等，供\code{wateros-syscall} socket系统调用读写。限制：无完整\code{select}/\code{epoll}与网卡多队列；中断未驱动收包，依赖poller轮询。
