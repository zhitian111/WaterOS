...
RISC-V 主线从设备树找 MMIO VirtIO 与 NS16550；LoongArch 主线走 PCIe 配置空间枚举 VirtIO-PCI，早期 UART 用固定 MMIO 窗口。块设备可选一层写穿缓存；网卡注册后由内置协议栈周期性轮询，再与用户态 socket 路径衔接。设备就绪后文件系统组件刷新设备目录，根卷探测才能看到块设备节点。当前 bring-up 仍以第一块盘、第一张网卡为主，多实例表已具备但未全面用到。
