...
\code{RoundRobinScheduler}仅维护单一\code{OtherReadyQueue}，所有可运行任务（含内核与用户）在同一轮转队列中调度。结构与 multi-class 共享\code{TaskRegistry}、\code{WaitQueues}、tick 计数与 arch 切换路径，但不拆分 FIFO/RR 就绪队列。
