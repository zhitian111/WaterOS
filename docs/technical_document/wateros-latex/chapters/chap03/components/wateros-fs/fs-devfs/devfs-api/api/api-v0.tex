...
\paragraph{类型。}
\code{DevNodeType}区分\code{Block}、\code{Character}、\code{Unsupported}。\code{DevNode}含逻辑路径（如\code{/dev/vda}）与类型，供启动日志与块设备路径解析。错误复用\code{fs\_api\_v0::FsError}/\code{FsResult}，与根卷挂载错误映射一致。
