...
\paragraph{错误与能力枚举。}
\code{FsError}统一描述卷操作失败语义，含\code{NotMounted}、\code{NotFound}、\code{NotAFile}、\code{InvalidPath}、\code{Exists}、\code{Unsupported}、\code{Driver}、\code{Corrupt}、\code{Io}、\code{NoSpace}等变体。\code{FsKind}标识文件系统种类（\code{Ext4}、\code{DevFs}、\code{Other}等）；\code{FsAccessMode}区分RO/RW；\code{FsCapability}为\code{(kind, access)}静态能力声明。\code{FsMetadata}、\code{FsDirEntry}、\code{FsNodeType}对齐Linux\code{stat}/\code{dirent}子集。
