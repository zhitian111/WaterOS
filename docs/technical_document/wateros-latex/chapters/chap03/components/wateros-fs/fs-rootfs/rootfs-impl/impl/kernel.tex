...
\paragraph{挂载流程。}
\code{mount\_root\_rw\_from\_block\_path}：\code{devfs::lookup\_block\_device} $\rightarrow$ \code{ACTIVE\_FS\_IMPL.mount\_rw} $\rightarrow$ 写入\code{ROOT\_RW\_FS}与\code{ROOT\_DEV\_PATH} $\rightarrow$ \code{mount\_generation}++。\code{mount\_default\_root\_rw}使用\code{default\_root\_block\_path()}（通常\code{/dev/vda}）。
