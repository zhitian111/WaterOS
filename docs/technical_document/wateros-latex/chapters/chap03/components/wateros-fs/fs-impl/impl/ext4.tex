...
\paragraph{定位。}
可选实现路径，RO/RW均基于\code{ext4plus}库（\code{ro.rs}、\code{rw.rs}）。在\code{registered\_fs\_impls()}中位于\code{impl-ext4-rs}之后；若两套feature同时启用，probe时\code{ext4-rs}优先匹配。

\paragraph{探测与注册。}
\code{probe\_ext4\_magic}逻辑与\code{impl-ext4-rs}相同（superblock \code{0xEF53}）。\code{Ext4FsImpl}导出\code{IMPL}，\code{name()}为\code{"ext4"}，声明\code{Ext4} RO与RW能力。
