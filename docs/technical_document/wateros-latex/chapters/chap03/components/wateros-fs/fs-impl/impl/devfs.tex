...
\paragraph{与注册表的关系。}
聚合\code{registered\_fs\_impls()}中的devfs项来自\code{devfs::active\_impl::IMPL}（\code{impl-kernel}），不是本crate。\code{wateros-fs}的\code{impl-devfs} feature目前仅将本crate列入工作区依赖图，\code{src/lib.rs}未直接\code{use}其符号。
