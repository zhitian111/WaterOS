...
\paragraph{类型与回调。}
\code{TaskId}与\code{task::TaskId}数值一致，api层不依赖\code{wateros-task}。\code{ProcMountLine}描述\code{/proc/mounts}单行（\code{device}、\code{mount\_point}、\code{fstype}）。三类函数指针回调：
\begin{itemize}
...
\begin{itemize}
  \item \code{exists}：已知proc节点（含目录与文件）返回\code{true}；
