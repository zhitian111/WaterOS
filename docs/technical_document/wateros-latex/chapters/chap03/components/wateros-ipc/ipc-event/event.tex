...
\code{ipc-event}为独立占位 crate，未列入\code{wateros-ipc} workspace members，父聚合\code{ipc.tex}虽通过 \texttt{\textbackslash input} 纳入本节，但根\code{wateros-ipc/Cargo.toml}无\code{dep:event}，\code{ipc::event}命名空间尚不存在。设计意图是预留事件对象与同步类 IPC 的 crate 边界，供后续 epoll 事件队列或内核句柄语义落地。
