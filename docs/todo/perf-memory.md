...
> 完整口径、项目特有风险（单核 RefCell 伪锁、TLB 静默错误、页表生命周期耦合）与安全实施流程见 [`perf-risk-assessment.md`](./perf-risk-assessment.md)。

| 编号 | 收益 | 风险 | 风险类型 | Flag | 关键验证 |
|------|------|------|----------|------|------|
| M-1 | 高 | 高 | TLB 一致性 | 是 | 增量（先同 aspace）+ 压测 + 断言 |
| M-2 | 高（前置） | 高 | TLB / 复用 ASID | 是 | 同 M-1；ASID 回绕用例 |
| M-3 | 高 | 高 | 页表生命周期 / break-COW | 是 | fork 压测 + 帧账本断言 |
| M-4 | 高 | 中高 | 回收正确性 | 建议 | exit 压测 + 帧账本断言 |
| M-5 | 高 | 高 | 误删共享/恒等节点 | 是 | mmap 抖动 + 断言 |
| M-6 | 高 | 中 | 安全清零（信息泄漏） | 建议 | 新页置零校验 |
| M-7 | 中高 | 中 | 放置正确性 | 否 | mmap/mremap LTP |
| M-8 | 中 | 低 | 行为保持（位图已覆盖） | 否 | 帧分配测例 |
| M-9 | 中 | 中高 | refcount + TLB | 建议 | fork 后写 LTP |
| M-10 | 中 | 中 | 缓存一致 | 建议 | 全量 + 大块拷贝 |
| M-11 | 中 | 低中 | 错误路径账本 | 否 | OOM 注入 |
| M-12 | 中低 | 中 | 引导顺序 | 否 | 启动验证 |
| M-13 | 中低 | 中 | 分配器正确性 | 是 | 全量 + 堆压测 |
| M-14 | 中低 | 中 | VMA 一致 | 否 | 多映射 mmap |
| M-15 | 中低 | 中 | 用户帧 vs 页缓存双份 | 建议 | mmap 文件读写 |
| M-16 | 中低 | 中高 | break COW | 建议 | mremap 测例 |
| M-17 | 低中 | 低 | 行为保持 | 否 | 编译 + fork |
| M-18 | 低（性能） | 正确性 P0 / 实现中 | 共享页回收 | 建议 | shared anon + 断言 |
| M-19 | 中 | 中 | 可见内存语义 | 建议 | brk/glibc 分配器 |
| M-20 | 中 | 中 | COW/lazy 顺序 | 建议 | fault 测例 |

低风险可先做：**M-8、M-17 已实施**（2026-06-28）。高风险须 Flag+灰度：M-1、M-2、M-3、M-5（且相互耦合，建议成组推进）。
