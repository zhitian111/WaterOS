...
> 完整口径、项目特有风险与安全实施流程见 [`perf-risk-assessment.md`](./perf-risk-assessment.md)。F-2/F-6/F-7 属「修现存丢脏 bug」，做了是净减风险。

| 编号 | 收益 | 风险 | 风险类型 | Flag | 关键验证 |
|------|------|------|----------|------|------|
| F-1 | 高 | 中 | 只读卷写回语义 | 建议 | aux 卷大文件读 |
| F-2 | 高（正确性） | 中 | 丢脏修复 | 否 | unlink 打开中文件 + 脏页 |
| F-3 | 高 | 中高 | 缓存失效（rename/unlink/mount） | 建议 | rename/unlink/mount 失效用例 |
| F-4 | 高 | 中 | 三表不变量 | 否(+断言) | 读写压测 |
| F-5 | 高 | 中 | 锁序 | 否 | 多线程 fsync |
| F-6 | 高 | 中 | 丢脏修复 | 否 | mount + 脏页 |
| F-7 | 高 | 中 | 写回覆盖面 | 否 | sync/fsync/fdatasync |
| F-8 | 高 | 中 | 锁外 IO 约定 | 建议 | 读 miss/驱逐压测 |
| F-9 | 高（LA） | 低中 | cfg 接线 | 是(cfg) | LA ext4 读写 |
| F-10 | 中 | 中 | 索引一致 | 否(+断言) | close/unlink 批量 |
| F-11 | 中 | 中 | 失效 | 否 | 多挂载点 |
| F-12 | 中 | 中 | 脏读（多 FS） | 否 | 多 FS 实例 |
| F-13 | 中 | 中 | 缓存一致 | 否 | 写后读 |
| F-14 | 中 | 低 | 仅影响预取 | 否 | 随机读 |
| F-15 | 中 | 中 | API 调用面 | 否 | 全量（grep 调用面） |
| F-16 | 中 | 低中 | 锁内嵌套 | 否 | 并发 metadata |
| F-17 | 中 | 中 | 缓存失效 | 否 | rename + open fd |
| F-18 | 中 | 低中 | dup 语义 | 否 | unlink 后写 + dup |
| F-19 | 低 | 低中 | 回收语义 | 否 | 长跑 create/unlink |
| F-20 | 低 | 低 | 仅可观测性 | 否 | 无 |
| F-21 | 低 | 低 | 可靠性 | 否 | 故障注入 |

低风险可先做：**F-14、F-20、F-21 已实施**（2026-06-28）；F-9 仍待做。修 bug 类优先：F-2、F-6、F-7。中高风险须配失效用例：F-3。
