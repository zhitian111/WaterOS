...
| `ltp_fast_exit_skip_list.md` | 分析类 | `general`、`debug_workflow`；另读 `ltp_cgroup_helper.rs` | 不需要 |
...
| `known-issues/*.md` | 当前问题实施与验收类 | `general`、`structure`、`coding`、`architecture` | 按任务涉及组件选读 |
| `perf/*.md` | 性能实施类 | `general`、`structure`、`coding`、`architecture` | 按需（见 `perf/README.md` 与各任务文件） |
| `read-family/*.md` | syscall/VFS/MM 正确性实施类 | `general`、`structure`、`coding`、`architecture` | 按任务涉及组件选读 |
...
- `ltp_fast_exit_skip_list.md`（**从 LTP 日志提取「未见 TPASS」fast exit 跳过名单**；主 agent 按 ~4000 行分片派 subagent 汇总）
...
- `known-issues/README.md`（**当前全部已知问题的统一入口**；不按成员划分，包含依赖、并行边界、现状证据和验收）
- `perf/README.md`（**性能优化 Agent 任务索引**；按 wave0~3 下发块缓存、fork/exit、FS、网络、trap 等实施任务）
- `read-family/README.md`（**`read/readv/pread` 调用族正确性任务索引**；按依赖拆分 MM、VFS、pipe、socket 和回归）
