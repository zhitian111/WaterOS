...
为每个组件导出“如何新增一个 impl”的说明，包括需要修改哪些 `Cargo.toml`、哪些 feature、需要实现哪些 trait 或类型，以及这些定义在何处。
...
本任务为导出类，输出写入 `docs/exports/impl-guide/`，**不需要**预先阅读现有导出结果（除非做增量对比）。
