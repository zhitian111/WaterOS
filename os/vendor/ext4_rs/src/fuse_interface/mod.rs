...
impl Ext4 {
    ...
    pub fn fuse_lookup(&self, parent : u64, name : &str) -> Result<FileAttr> {
        ...
        let inode_num = search_result.dentry
                                     .inode;
    ...
    /// Create a regular file, character device, block device, fifo or socket node.
    pub fn fuse_mknod_with_attr(&self,
                                parent : u64,
                                name : &str,
                                mode : u32,
                                umask : u32,
        ...
        let inode_ref = self.create_with_attr(parent as u32,
                                              name,
                                              mode as u16,
                                              uid as u16,
                                              gid as u16)?;
    ...
    pub fn fuse_mkdir(&mut self,
        ...
        let file_type = match InodeFileType::from_bits(mode as u16 & EXT4_INODE_MODE_TYPE_MASK) {
            Some(file_type) => file_type,
            None => return_errno_with_message!(Errno::EINVAL,
                                               "Invalid mode for directory creation"),
        };
    ...
    /// Create a directory.
    pub fn fuse_mkdir_with_attr(&mut self,
                                parent : u64,
                                name : &str,
                                mode : u32,
                                umask : u32,
        ...
        let mode = file_type.bits() | (mode as u16 & EXT4_INODE_MODE_PERM_MASK);
        let inode_ref = self.create_with_attr(parent as u32,
                                              name,
                                              mode,
                                              uid as u16,
    ...
    pub fn fuse_unlink(&self, parent : u64, name : &str) -> Result<usize> {
        ...
        let mut nameoff = 0;
        let child_inode = self.generic_open(name,
                                            &mut parent_inode,
                                            false,
                                            0,
                                            &mut nameoff)?;
    ...
    pub fn fuse_rmdir(&mut self, parent : u64, name : &str) -> Result<usize> {
        ...
        let mut child_inode_ref = self.get_inode_ref(search_result.dentry
                                                                  .inode);

        self.truncate_inode(&mut child_inode_ref, 0)?;

        self.unlink(&mut parent_inode_ref,
                    &mut child_inode_ref,
                    name)?;
    ...
    pub fn fuse_symlink(&mut self, parent : u64, link_name : &str, target : &str) -> Result<usize> {
        let mut search_result = Ext4DirSearchResult::new(Ext4DirEntry::default());
        let r = self.dir_find_entry(parent as u32,
                                    link_name,
                                    &mut search_result);
        if r.is_ok() {
            return_errno!(Errno::EEXIST);
    ...
    pub fn fuse_open(&mut self, ino : u64, flags : i32) -> Result<usize> {
        ...
        // check permission
        let file_type = inode_ref.inode
                                 .file_type();
        let file_perm = inode_ref.inode
                                 .file_perm();
    ...
    pub fn fuse_create(&mut self,
        ...
        let r = self.dir_find_entry(parent as u32, name, &mut search_result);
        if r.is_ok() {
            let inode_ref = self.get_inode_ref(search_result.dentry
                                                            .inode);
    ...
    pub fn fuse_access(&mut self, ino : u64, uid : u16, gid : u16, mode : u16, mask : i32) -> bool {
        ...
        inode_ref.inode
                 .check_access(uid, gid, mode, mask as u16)
    }
    ...
    pub fn fuse_init(&mut self) -> Result<usize> { Ok(EOK) }

    /// Clean up filesystem.
    /// Called on filesystem exit.
    pub fn fuse_destroy(&mut self) -> Result<usize> { Ok(EOK) }

    /// Rename a file.
    fn fuse_rename(&mut self,
                   parent : u64,
                   name : &str,
                   newparent : u64,
                   newname : &str,
    ...
    fn fuse_copy_file_range(&mut self,
                            ino_in : u64,
                            fh_in : u64,
                            offset_in : i64,
                            ino_out : u64,
