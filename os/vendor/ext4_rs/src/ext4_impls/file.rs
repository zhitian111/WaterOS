...
impl Ext4 {
    ...
    pub fn link(&self,
        ...
        if child.inode.is_dir() {
            ...
            let new_child_ref = Ext4InodeRef { inode_num : child.inode_num,
                                               inode : child.inode };
            ...
            child.inode
                 .set_links_count(2);
            let link_cnt = parent.inode
                                 .links_count() +
                           1;
            parent.inode
                  .set_links_count(link_cnt);

            return Ok(EOK);
        }

        // Increment the link count of the child inode
        let link_cnt = child.inode
                            .links_count() +
                       1;
        child.inode
             .set_links_count(link_cnt);
    ...
    pub fn create(&self, parent : u32, name : &str, inode_mode : u16) -> Result<Ext4InodeRef> {
        ...
        self.link(&mut parent_inode_ref,
                  &mut child_inode_ref,
                  name)?;
    ...
    pub fn create_inode(&self, inode_mode : u16) -> Result<Ext4InodeRef> {
        ...
        let inode_size = self.super_block
                             .inode_size();
        let extra_size = self.super_block
                             .extra_size();
    ...
    pub fn read_at(&self, inode : u32, offset : usize, read_buf : &mut [u8]) -> Result<usize> {
        ...
        if offset + read_buf_len > file_size as usize {
            ...
            log::trace!("[Read] Adjusted read size to {} bytes to not exceed file size (offset: \
                         {}, file_size: {})",
                        read_buf_len,
                        offset,
                        file_size);
        ...
        if unaligned_start_offset > 0 {
            let adjust_read_size = min(BLOCK_SIZE - unaligned_start_offset,
                                       read_buf_len);
        ...
        while total_bytes_read < read_buf_len && iblock < iblock_last {
            let mut read_length = min(BLOCK_SIZE,
                                      read_buf_len - total_bytes_read);
    ...
    pub fn write_at(&self, inode : u32, offset : usize, write_buf : &[u8]) -> Result<usize> {
        ...
        let file_size = inode_ref.inode
                                 .size();
        log::trace!("[Write] Starting write - inode: {}, offset: {}, size: {}, current file \
                     size: {}",
                    inode,
                    offset,
                    write_buf_len,
        ...
        if unaligned > 0 {
            log::trace!("[Alignment] Unaligned start: {} bytes",
                        unaligned);
        }
        ...
        let existing_blocks_from_start = if iblk_idx < ifile_blocks as usize {
            ifile_blocks as usize - iblk_idx
        } else {
            0
        };
        let blocks_to_allocate = total_blocks_needed.saturating_sub(existing_blocks_from_start);

        if blocks_to_allocate > 0 {
            log::trace!("[Pre-allocation] Allocating {} blocks",
                        blocks_to_allocate);

            // 使用append_inode_pblk_batch进行批量块分配
            let allocated_blocks = self.append_inode_pblk_batch(&mut inode_ref,
                                                                &mut start_bgid,
                                                                blocks_to_allocate)?;

            // If we couldn't allocate all blocks, adjust the write size
            if allocated_blocks.len() < blocks_to_allocate {
                log::trace!("[Write] Could only allocate {} out of {} blocks",
                ...
                if adjusted_write_size == 0 {
                    ...
                    return return_errno_with_message!(Errno::ENOSPC,
                                                      "No blocks available for write");
                }

                // Update write size
                write_buf_len = min(write_buf_len, adjusted_write_size);
                log::trace!("[Write] Adjusted write size from {} to {} bytes",
                            write_buf.len(),
                            write_buf_len);
            }
        ...
        let available_blocks = existing_blocks_from_start + new_blocks;
        ...
        if unaligned > 0 && written < write_buf_len {
            ...
            log::trace!("[Unaligned Write] Writing {} bytes",
                        len);
            ...
            let pblock_idx = match self.get_pblock_idx(&inode_ref, iblk_idx as u32) {
                Ok(idx) => idx,
                Err(e) => {
                    log::error!("[Write] Failed to get physical block for logical block {}: {:?}",
                                iblk_idx,
                                e);
                    return Err(e);
                }
            ...
            let verify_block = Block::load(&self.block_device,
                                           pblock_idx as usize * BLOCK_SIZE);
            if verify_block.data[unaligned..unaligned + len] != write_buf[..len] {
                log::error!("[Write] Verification failed for unaligned write at block {}",
                            pblock_idx);
        ...
        log::info!("[Aligned Write] Starting aligned writes for {} blocks",
        ...
        while written < write_buf_len {
            ...
            block.write_offset(0,
                               &write_buf[written..written + write_size],
                               write_size);
            ...
            if verify_block.data[..write_size] != write_buf[written..written + write_size] {
                log::error!("[Write] Verification failed for aligned write at block {}",
                            pblock_idx);
                return return_errno_with_message!(Errno::EIO, "Write verification failed");
            }
            drop(block);
            ...
            if aligned_blocks % 1000 == 0 {
                log::trace!("[Progress] Written {} blocks, {} bytes",
                            aligned_blocks,
                            written);
            }
        }
        ...
        if new_size > file_size as usize {
               ...
               new_size as u64
            {
                log::error!("[Write] File size update verification failed: expected {}, got {}",
                            ...
                            verify_inode.inode
                                        .size());
                return return_errno_with_message!(Errno::EIO,
    ...
    pub fn file_remove(&self, path : &str) -> Result<usize> {
        ...
        let mut nameoff = 0;
        let child_inode = self.generic_open(path,
        ...
        let child_link_cnt = child_inode_ref.inode
                                            .links_count();
        if child_link_cnt == 1 {
        ...
        let r = self.unlink(&mut parent_inode_ref,
                            &mut child_inode_ref,
                            &p[..len])?;
    ...
    pub fn truncate_inode(&self, inode_ref : &mut Ext4InodeRef, new_size : u64) -> Result<usize> {
        ...
        if diff_blocks_cnt > 0 {
            self.extent_remove_space(inode_ref,
                                     new_blocks_cnt,
                                     EXT_MAX_BLOCKS)?;
        }
