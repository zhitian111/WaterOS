...
impl Ext4 {
    ...
    pub fn try_insert_to_existing_block(
        ...
        while offset < BLOCK_SIZE - size_of::<Ext4DirEntryTail>() {
            ...
            if rec_len == 0 {
                return_errno_with_message!(Errno::ENOSPC, "zero-length directory entry");
            }

            if de.unused() {
                if rec_len as usize >= required_len {
                    let mut new_entry = Ext4DirEntry::default();
                    let de_type = DirEntryType::EXT4_DE_DIR;
                    new_entry.write_entry(rec_len, child_inode, name, de_type);
