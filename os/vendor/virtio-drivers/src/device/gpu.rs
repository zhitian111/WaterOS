...
impl<H: Hal, T: Transport> VirtIOGpu<H, T> {
    ...
    /// Flush a rectangular part of the framebuffer to the host.
    ///
    /// The framebuffer backing store is a tightly packed 32-bit image whose
    /// row stride is the scanout width.  Rejecting empty and out-of-bounds
    /// rectangles here keeps every transport user on the same safe path.
    pub fn flush_region(
        &mut self,
        x: u32,
        y: u32,
        ...
        let full = self.rect.ok_or(Error::NotReady)?;
        let (rect, offset) = checked_flush_region(full, x, y, width, height)?;
...
fn checked_flush_region(
    ...
    let x_end = x.checked_add(width).ok_or(Error::InvalidParam)?;
    let y_end = y.checked_add(height).ok_or(Error::InvalidParam)?;
    if x < full.x
        || y < full.y
        || x_end > full.x.checked_add(full.width).ok_or(Error::InvalidParam)?
        || y_end > full.y.checked_add(full.height).ok_or(Error::InvalidParam)?
    {
        return Err(Error::InvalidParam);
    }
    let relative_x = x.checked_sub(full.x).ok_or(Error::InvalidParam)?;
    let relative_y = y.checked_sub(full.y).ok_or(Error::InvalidParam)?;
    let pixel_offset = u64::from(relative_y)
        .checked_mul(u64::from(full.width))
        .and_then(|value| value.checked_add(u64::from(relative_x)))
        .and_then(|value| value.checked_mul(4))
        .ok_or(Error::InvalidParam)?;
    Ok((Rect { x, y, width, height }, pixel_offset))
}

#[cfg(test)]
mod region_tests {
    use super::{checked_flush_region, Rect};

    #[test]
    fn computes_linear_backing_offset() {
        let full = Rect { x: 0, y: 0, width: 1280, height: 800 };
        let (rect, offset) = checked_flush_region(full, 10, 20, 640, 400).unwrap();
        assert_eq!((rect.x, rect.y, rect.width, rect.height), (10, 20, 640, 400));
        assert_eq!(offset, ((20 * 1280 + 10) * 4) as u64);
    }

    #[test]
    fn rejects_empty_overflow_and_out_of_bounds_regions() {
        let full = Rect { x: 0, y: 0, width: 1280, height: 800 };
        assert!(checked_flush_region(full, 0, 0, 0, 1).is_err());
        assert!(checked_flush_region(full, 1200, 0, 100, 1).is_err());
        assert!(checked_flush_region(full, u32::MAX, 0, 2, 1).is_err());
    }
}
