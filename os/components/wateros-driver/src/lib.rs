...
//!
//! `api`、`block`、`character`、`network` 模块不做按子系统裁剪，便于上层一次性依赖；具体硬件绑定由 feature 选中的 [`active_impl`] 完成。
...
/// 合并各子系统 [`supported_devices()`] 的静态条目，便于诊断「内核声明了哪些可绑定设备」。
pub fn supported_device_entries() -> Vec<&'static SupportedDeviceEntry> {
    block::supported_devices()
        .iter()
...
/// 引导早期调用：保存 DTB 物理地址等平台状态；具体解析在 [`init_after_boot`] 或各实现内完成。
...
/// 物理 RAM 上界（不包含），用于 Sv39 恒等映射与帧分配器；QEMU 实现从 DTB 解析，其它配置返回回退常量。
#[inline]
...
/// 内核完成必要子系统初始化后调用：扫描/注册设备等；失败时记录日志，不向上返回错误（当前契约）。
...
/// 自检入口：依次调用 API 与各子系统测试钩子；QEMU 实现会跑探测路径，dummy 实现跳过硬件。
pub fn test() {
