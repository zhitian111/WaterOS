...
//!
//! The protocol is testable without a board through [`SdTransport`]. Physical
//! clock/reset, pinmux, voltage and card-detect behavior remain board concerns.
use crate::mmc::{DwMmc, MmcError, RegisterIo};
use alloc::{boxed::Box, sync::Arc};
use block::{BlockDevice, DriverError, DriverResult, Lba, SharedBlockDevice, BLOCK_SIZE};
use spin::Mutex;
...
impl<T : SdTransport + Send> BlockDevice for SdCard<T> {
    ...
    fn read_blocks(&mut self, start_block : Lba, output : &mut [u8]) -> DriverResult<()> {
        ...
        for (offset, block) in output.chunks_exact_mut(BLOCK_SIZE)
                                     .enumerate()
        {
            ...
            self.transport
                .read_single_block(argument, block)
                .map_err(map_error)?;
        }
...
pub fn parse_csd_total_blocks(csd : [u32; 4]) -> Result<u64, MmcError> {
    let raw = csd.into_iter()
                 .fold(0u128, |value, word| {
                     (value << 32) | word as u128
                 });
...
mod tests {
    ...
    impl SdTransport for ScriptedCard {
        ...
        fn read_single_block(&mut self,
            ...
            self.reads
                .push(argument);
            for (index, byte) in output.iter_mut()
                                       .enumerate()
            {
    ...
    fn csd_v2(size : u32) -> [u32; 4] { csd_words((1u128 << 126) | ((size as u128) << 48)) }
    ...
    fn common_prefix(cmd8 : Result<CommandResponse, MmcError>,
                     acmd41_argument : u32,
                     ocr : u32,
                     csd : [u32; 4])
                     -> Vec<ExpectedCommand> {
        vec![command(0,
             ...
             ExpectedCommand { index : 8,
                               argument : 0x1AA,
                               kind : ResponseKind::Short,
                               result : cmd8 },
             command(55,
                     0,
                     ResponseKind::Short,
             ...
             command(3,
                     0,
                     ResponseKind::Short,
                     CommandResponse::Short(0x1234_0000)),
             command(9,
                     0x1234_0000,
                     ResponseKind::Long,
                     CommandResponse::Long(csd)),
             command(7,
                     0x1234_0000,
                     ResponseKind::Short,
                     CommandResponse::Short(0))]
    }
    ...
    fn initializes_sdhc_and_uses_block_addressing() {
        let commands =
            common_prefix(Ok(CommandResponse::Short(0x1AA)),
                          ...
                          OCR_CARD_POWERED_UP | OCR_CARD_CAPACITY_STATUS | OCR_VOLTAGE_27_TO_36_V,
                          csd_v2(0x12345));
        let mut card = SdCard::initialize(ScriptedCard::new(commands), 2).unwrap();
        assert_eq!(card.info()
                       .addressing,
                   CardAddressing::Block);
        assert_eq!(card.total_blocks(),
                   Some((0x12345u64 + 1) * 1024));
        let mut data = [0; BLOCK_SIZE * 2];
        card.read_blocks(Lba(7), &mut data)
            .unwrap();
        assert_eq!(&data[..4], &[7, 8, 9, 10]);
        assert_eq!(&data[BLOCK_SIZE..BLOCK_SIZE + 4], &[8,
                                                        9,
                                                        10,
                                                        11]);
    ...
    fn rejects_address_overflow_bad_buffers_and_accepts_writes() {
        let commands =
            common_prefix(Ok(CommandResponse::Short(0x1AA)),
                          ...
                          csd_v2(0x3F_FFFF));
        let mut card = SdCard::initialize(ScriptedCard::new(commands), 1).unwrap();
        assert_eq!(card.read_blocks(Lba(u32::MAX as u64 + 1),
                                    &mut [0; BLOCK_SIZE]),
                   Err(DriverError::InvalidParam));
    ...
    fn parses_csd_versions_and_rejects_reserved_structure() {
        assert_eq!(parse_csd_total_blocks(csd_v2(0x3F_FFFF)),
                   Ok(1u64 << 32));
        assert_eq!(parse_csd_total_blocks(csd_v1(1023, 3, 9)),
                   Ok(32768));
        assert_eq!(parse_csd_total_blocks(csd_words(2u128 << 126)),
                   Err(MmcError::Response));
        assert_eq!(parse_csd_total_blocks(csd_v1(1, 0, 8)),
                   Err(MmcError::Response));
    }
