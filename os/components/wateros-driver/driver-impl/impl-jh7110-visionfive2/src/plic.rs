//! RISC-V PLIC 寄存器算术与 MMIO 访问（构造不触达硬件）。
...
impl PlicMmio {
    pub fn new(description : PlicDescription, context : usize) -> DriverResult<Self> {
        if context >= description.contexts.len() || description.sources == 0 {
            return Err(DriverError::InvalidParam);
        }
        let end = 0x20_0004usize.checked_add(context.checked_mul(0x1000)
                                                    .ok_or(DriverError::InvalidParam)?)
                                .ok_or(DriverError::InvalidParam)?;
        if description.mmio.size < end + 4 {
            return Err(DriverError::InvalidParam);
        }
        Ok(Self { description, context })
    }

    pub fn claim_complete_offset(&self) -> usize {
        0x20_0004 + self.context * 0x1000
    }

    pub fn threshold_offset(&self) -> usize {
        0x20_0000 + self.context * 0x1000
    }
...
mod tests {
    ...
    fn validates_standard_offsets() {
        let d = PlicDescription {
            mmio : MmioRegion { base : 0xC00_0000, size : 0x40_0000 },
            sources : 136,
        ...
        assert_eq!(plic.enable_offset(32).unwrap(), 0x2004);
        assert_eq!(plic.claim_complete_offset(), 0x20_0004);
        assert!(plic.enable_offset(137).is_err());
    }
