//! JH7110 PLIC 分发与 handler 生命周期。
//!
//! DTB 发现本身不会使能任何中断源。设备驱动必须在注销前先静默设备侧 IRQ/DMA；
//! 寄存器访问、路由与 cache/device 顺序仍待真机测试。
...
pub fn initialize_current_hart(hart_id : usize) -> DriverResult<()> {
    ...
    #[cfg(target_arch = "riscv64")]
    {
        ...
        log::info!("[driver][visionfive2] enabled supervisor external interrupts hart={} context={}",
                   hart_id,
                   context);
        Ok(())
    }
...
pub fn unregister_irq_handler(lease : &IrqLease) -> DriverResult<bool> {
    ...
    // 不要在 handler 内部调用注销；设备侧静默 + 此 drain 保证返回时没有残留 handler。
...
pub fn handle_external_interrupt(hart_id : usize) -> DriverResult<bool> {
    ...
    if let Some(dispatch) = handler_for(source) {
        ...
        unsafe { plic.disable_source(source)?; }
        log::warn!("[driver][visionfive2] unregistered PLIC source {}; disabled and completing it",
                   source);
    }
...
mod tests {
    ...
    fn record(source : u32) {
        HANDLED.store(source, Ordering::Release);
    }

    fn blocking_record(_source : u32) {
        BLOCKING_STARTED.store(true, Ordering::Release);
        while !BLOCKING_RELEASE.load(Ordering::Acquire) {
            core::hint::spin_loop();
        }
    }
