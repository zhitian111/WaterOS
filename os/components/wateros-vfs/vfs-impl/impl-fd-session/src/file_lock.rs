...
impl InodeLocks {
    // 本方法代码由AI完成
    fn new() -> Arc<Self> {
        Arc::new(Self { data : Mutex::new(InodeLockData { posix : Vec::new(),
...
pub fn inode_key_from_metadata(meta : &VfsMetadata) -> Option<InodeKey> {
    ...
    Some(InodeKey { mount_id : meta.mount_id,
                    inode : meta.inode })
}
...
// 本方法代码由AI完成
fn intersection_range(probe_start : u64,
                      probe_len : u64,
                      lock_start : u64,
                      lock_len : u64)
                      -> Option<(u64, u64)> {
...
// 本方法代码由AI完成
fn find_getlk_conflict(locks : &[PosixLock],
                       pid : ProcessId,
                       probe_typ : LockType,
                       probe_start : u64,
                       probe_len : u64)
    ...
    for lock in locks {
        ...
        let Some((isect_start, isect_len)) = intersection_range(probe_start,
                                                                probe_len,
                                                                lock.start,
                                                                lock.len)
        else {
        ...
        if best.as_ref()
               .is_none_or(|(_, best_start, _)| isect_start < *best_start)
        {
...
// 本方法代码由AI完成
fn posix_conflict(locks : &[PosixLock],
                  pid : ProcessId,
                  typ : LockType,
                  start : u64,
                  len : u64)
...
fn flock_blocks_posix(flock : &FlockState, typ : LockType) -> bool {
    match typ {
        LockType::Read => flock.exclusive
                               .is_some(),
        LockType::Write => {
            flock.exclusive
                 .is_some() ||
            !flock.shared_holders
                  .is_empty()
        }
    }
}
...
fn split_lock_around_region(lock : PosixLock, cut_start : u64, cut_len : u64) -> Vec<PosixLock> {
    ...
    if lock.start < cut_start {
        out.push(PosixLock { pid : lock.pid,
                             typ : lock.typ,
                             start : lock.start,
...
// 本方法代码由AI完成
fn remove_posix_for_pid_region(locks : &mut Vec<PosixLock>,
                               pid : ProcessId,
                               start : u64,
                               len : u64) {
...
// 本方法代码由AI完成
fn apply_posix_lock(locks : &mut Vec<PosixLock>,
                    pid : ProcessId,
                    typ : LockType,
                    start : u64,
                    len : u64) {
    remove_posix_for_pid_region(locks, pid, start, len);
    locks.push(PosixLock { pid,
                           typ,
                           start,
                           len });
}
...
// 本方法代码由AI完成
fn posix_would_block(data : &InodeLockData,
                     pid : ProcessId,
                     typ : LockType,
                     start : u64,
                     len : u64)
...
// 本方法代码由AI完成
pub fn posix_getlk(key : &InodeKey, pid : ProcessId, flock : &mut Flock) -> VfsResult<()> {
...
// 本方法代码由AI完成
pub fn posix_setlk(key : &InodeKey,
                   pid : ProcessId,
                   flock : &Flock,
                   blocking : bool)
                   -> VfsResult<()> {
...
pub fn release_process_inode_locks(pid : ProcessId, key : &InodeKey) {
    let Some(locks) = LOCK_TABLE.lock()
                                .get(key)
                                .cloned()
    else {
        return;
...
/// 进程最后一个线程退出时，释放其在**全部** inode 上持有的 POSIX 记录锁。
///
/// Linux 语义：POSIX 记录锁属于进程（pid），进程结束即全部自动释放。WaterOS
/// 的锁表是全局静态表，若只依赖 `close` 路径释放，孤儿进程（父进程不 reap）会
/// 永久残留锁，导致后续同文件加锁被 `pid` 挡住（如 dpkg frontend lock）。
pub fn release_process_all_locks(pid : ProcessId) {
    let mut drained = Vec::new();
    {
        let mut table = LOCK_TABLE.lock();
        for (key, locks) in table.iter() {
            let mut data = locks.data.lock();
            let before = data.posix.len();
            data.posix
                .retain(|lock| lock.pid != pid);
            if data.posix.len() != before {
                drained.push((*key, locks.clone()));
            }
        }
    }
    for (key, locks) in drained {
        locks.wait
             .wake_all();
        drop_inode_if_empty(&key, &locks);
    }
