...
static URANDOM_STATE : Mutex<UrandomState> = Mutex::new(UrandomState { value:
...
impl VfsPreparedRead for UrandomPreparedRead {
    fn acquire(self: Box<Self>) -> VfsResult<Box<dyn VfsReadLease>> {
        let (id, start) = {
            ...
            state.next_read_id = state.next_read_id
                                      .wrapping_add(1);
        ...
        let mut data = Vec::new();
        if data.try_reserve_exact(self.max_len)
               .is_err()
        {
        ...
        Ok(Box::new(UrandomReadLease { id,
                                       start,
                                       data,
                                       active : true }))
    }
...
impl VfsIoHandle for ConsoleInHandle {
    ...
    // 本方法代码由AI完成
    fn metadata(&self) -> VfsResult<VfsMetadata> { Ok(special_meta(0o20666, 1)) }
...
impl NullDeviceHandle {
    pub const fn new(accmode : u32, inode : u64) -> Self { Self { accmode, inode } }
}
...
impl VfsIoHandle for UrandomDeviceHandle {
    ...
    fn read(&mut self, buf : &mut [u8]) -> VfsResult<usize> {
        ...
        match lease.finish(VfsCopyProgress { copied : len,
                                             complete : true })?
        {
...
pub(crate) fn read_lease_self_test() {
    ...
    assert_eq!(lease.finish(VfsCopyProgress { copied : 1,
                                              complete : false }),
    ...
    let cancelled = random.prepare_read(4)
                          .expect("prepare random read")
                          .acquire()
                          .expect("acquire random read");
    let first = cancelled.bytes()
                         .to_vec();
    assert_eq!(cancelled.finish(VfsCopyProgress { copied : 0,
                                                  complete : false }),
               Ok(VfsReadFinish::Fault));
    let partial = random.prepare_read(4)
                        .expect("prepare random retry")
                        .acquire()
                        .expect("acquire random retry");
    assert_eq!(partial.bytes(), first.as_slice());
    assert_eq!(partial.finish(VfsCopyProgress { copied : 2,
                                                complete : false }),
               Ok(VfsReadFinish::Bytes(2)));
    assert_eq!(URANDOM_STATE.lock()
                            .value,
               urandom_advance(START, 2));

    let concurrent = random.prepare_read(2)
                           .expect("prepare random concurrent write")
                           .acquire()
                           .expect("acquire random concurrent write");
    let before_write = URANDOM_STATE.lock()
                                    .value;
    let entropy = b"mix";
    assert_eq!(random.write(entropy), Ok(entropy.len()));
    assert_eq!(concurrent.finish(VfsCopyProgress { copied : 2,
                                                   complete : true }),
               Ok(VfsReadFinish::Bytes(2)));
    assert_eq!(URANDOM_STATE.lock()
                            .value,
...
pub fn open_named_pipe(metadata : VfsMetadata,
    ...
    let (read_end, write_end) = if wants_read && wants_write {
        let read = named.open_read(true)
                        .map_err(map_pipe_err)?;
        let write = named.open_write(true)
                         .map_err(map_named_pipe_open_error)?;
        read.set_nonblocking(nonblocking);
        write.set_nonblocking(nonblocking);
        (Some(read), Some(write))
    } else if wants_read {
        (Some(named.open_read(nonblocking)
                   .map_err(map_pipe_err)?),
         None)
    } else if wants_write {
        (None,
         Some(named.open_write(nonblocking)
                   .map_err(map_named_pipe_open_error)?))
    } else {
        return Err(VfsError::InvalidPath);
    };

    Ok(Box::new(NamedPipeHandle { named,
                                  registry_key : key,
                                  read_end,
                                  write_end,
                                  metadata }))
...
impl Drop for NamedPipeHandle {
    fn drop(&mut self) {
        ...
        let mut registry = NAMED_PIPES.lock();
        let owns_entry = registry.get(&self.registry_key)
...
// 本方法代码由AI完成
pub fn pipe_handle_pair_with_flags(nonblocking : bool,
                                   direct : bool)
    ...
    (PipeReadHandle { endpoint : read,
                      inode },
     PipeWriteHandle { endpoint : write,
                       inode })
}
...
impl VfsIoHandle for NamedPipeHandle {
    ...
    fn prepare_read(&mut self, max_len : usize) -> VfsResult<Box<dyn VfsPreparedRead>> {
        let endpoint = self.read_end
                           .as_ref()
                           .ok_or(VfsError::BadFd)?;
        Ok(Box::new(PipePreparedRead { endpoint : endpoint.clone(),
                                       max_len }))
    }
    ...
    fn poll_revents(&mut self, events : i16) -> VfsResult<i16> {
        ...
        if events & POLLIN != 0 {
            if let Some(read) = &self.read_end {
                revents |= read.poll_revents(POLLIN)
                               .map_err(map_pipe_err)?;
            }
        }
        if events & POLLOUT != 0 {
            if let Some(write) = &self.write_end {
                revents |= write.poll_revents(POLLOUT)
                                .map_err(map_pipe_err)?;
            }
        }
        Ok(revents)
    ...
    fn poll_wait_for_ticks(&mut self,
        ...
        if events & POLLIN != 0 {
            if let Some(read) = &self.read_end {
                return read.poll_wait_for_ticks(POLLIN, timeout_ticks, still_waiting)
                           .map_err(map_pipe_err);
            }
        }
        if events & POLLOUT != 0 {
            if let Some(write) = &self.write_end {
                return write.poll_wait_for_ticks(POLLOUT, timeout_ticks, still_waiting)
                            .map_err(map_pipe_err);
            }
        }
        Ok(())
    ...
    fn duplicate(&self) -> VfsResult<Box<dyn VfsIoHandle>> {
        Ok(Box::new(Self { named : self.named
                                       .clone(),
                           registry_key:
                               self.registry_key,
                           read_end:
    ...
    fn open_status_flags(&self) -> u32 {
        let endpoint = self.read_end
                           ...
                           .or(self.write_end
                                   .as_ref());
        endpoint.filter(|endpoint| endpoint.nonblocking())
...
impl VfsIoHandle for PipeReadHandle {
    ...
    fn prepare_read(&mut self, max_len : usize) -> VfsResult<Box<dyn VfsPreparedRead>> {
        Ok(Box::new(PipePreparedRead { endpoint : self.endpoint
                                                      .clone(),
                                       max_len }))
    }
    ...
    // 本方法代码由AI完成
    fn metadata(&self) -> VfsResult<VfsMetadata> { Ok(special_meta(0o10600, self.inode)) }
    ...
    fn set_open_status_flags(&mut self, flags : u32) -> VfsResult<()> {
        self.endpoint
            .set_nonblocking(flags & O_NONBLOCK != 0);
        self.endpoint
            .set_direct(flags & O_DIRECT != 0);
        Ok(())
    }
...
pub fn stream_pair_handle_pair(nonblocking : bool) -> (UnixStreamPairEnd, UnixStreamPairEnd) {
    ...
    (UnixStreamPairEnd { read_end : read_ba,
                         write_end : write_ab,
                         inode },
     UnixStreamPairEnd { read_end : read_ab,
                         write_end : write_ba,
                         inode })
}
...
impl VfsIoHandle for UnixStreamPairEnd {
    ...
    fn poll_revents(&mut self, events : i16) -> VfsResult<i16> {
        ...
        if events & POLLIN != 0 {
            revents |= self.read_end
                           .poll_revents(POLLIN)
                           .map_err(map_pipe_err)?;
        }
        if events & POLLOUT != 0 {
            revents |= self.write_end
                           .poll_revents(POLLOUT)
                           .map_err(map_pipe_err)?;
        }
        Ok(revents)
    }
    ...
    // 本方法代码由AI完成
    fn metadata(&self) -> VfsResult<VfsMetadata> { Ok(special_meta(0o140600, self.inode)) }

    // 本方法代码由AI完成
    fn duplicate(&self) -> VfsResult<Box<dyn VfsIoHandle>> {
        Ok(Box::new(Self { read_end:
                               self.read_end
                                   .clone(),
                           write_end:
                               self.write_end
...
impl VfsReadLease for PipeVfsReadLease {
    fn bytes(&self) -> &[u8] { self.lease.bytes() }

    fn finish(self: Box<Self>, progress : VfsCopyProgress) -> VfsResult<VfsReadFinish> {
        let Self { lease } = *self;
        match lease.finish(progress.copied, progress.complete)
                   .map_err(map_pipe_err)?
        {
