...
impl PlatformSmp for QemuLoongArchSmp {
    ...
    fn start_cpu(cpu: CpuId, start_addr: usize, _: usize) -> PlatformSmpResult<()> {
        ...
        match CPU_STATES[cpu.raw()].compare_exchange(
            CPU_STOPPED,
            CPU_START_PENDING,
            Ordering::AcqRel,
            Ordering::Acquire,
            ...
            Err(other) => {
                return Err(PlatformSmpError::Firmware(
                    other as usize,
                ))
            }
        }
    ...
    fn cpu_status(cpu: CpuId) -> PlatformSmpResult<HartStatus> {
        ...
        Ok(
            match CPU_STATES[cpu.raw()].load(Ordering::Acquire) {
    ...
    fn configured_cpu_mask() -> CpuMask {
        CpuMask::from_bits((1u64 << config::task::MAX_CPUS) - 1)
    }
