...
pub extern "C" fn trap_entry_rust(cx_ptr : *mut TrapContext) {
    ...
    match trap_cause {
        ...
        TrapCause::Exception(Exception::StorePageFault) => {
            logging::debug!("[trap] page fault: cause={:?} scause={:#x?} sepc={:#x?} stval={:#x?}",
                            trap_cause,
                            cx.scause,
                            cx.sepc,
                            cx.stval);
        ...
        TrapCause::Interrupt(Interrupt::SupervisiorTimer) => {
            let now = super::time::Riscv64ArchTime::read_time_tick().expect("read time tick \
                                                                             during trap")
                                                                    .0;
    ...
    if cx.returns_to_user() {
        logging::trace!("[trap] return to user pc={:#x} sp={:#x}",
                        cx.user_pc(),
                        cx.user_sp());
    }
