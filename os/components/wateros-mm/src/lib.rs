...
//! 暴露的 bring-up 能力；具体 Sv39/桩代码 **不** 以 `mm_impl`
...
//! - [`kernel_mm`] 下 Sv39 实现依赖 **恒等映射或等价的物理线性访问**，以便页表
//!   walk 时把 PPN 当可写指针用；更换映射模型时需同步改 `mm-impl`。
//!
//! ## 与 trap / 执行态的关系
//!
//! - 内核全局页表由 [`kernel_mm`] 路径安装 `satp` 后，trap 与内核代码仍在 **S
//!   态** 下使用同一套映射（见 `kernel_global::init`
//!   文档）；用户态任务切换时再换 `satp`。
//!
//! ## Feature 与桩路径
//!
//! - 默认组合为 `impl-sv39` + 帧 `impl-stack`；仅启用 `impl-dummy`（mm 或
//!   frame）时对应子 crate 的 `//!`
//!   说明当前无操作/固定错误语义，便于在主机或未接线目标上通过编译。
...
pub mod kernel_mm {
    ...
    // 仅依赖 `impl-sv39`：根 crate 的 `qemu-riscv64-opensbi`
    // 不会自动为依赖包打开同名 feature， 若此处再要求 `mm/
    // qemu-riscv64-opensbi`，则从未启用该 flag 的构建会始终落到 dummy，
