...
fn map_segment<A: AddressSpaceOps>(
    ...
    while vpn.0 < vpn_end.0 {
        if let Some(_pa) = aspace
            .translate_addr(vpn.start_addr())
            .map_err(LoadElfError::Mm)?
        {
            // 与上一段 PT_LOAD 共享页（例如 text/data 尾与 .bss 头同 VPN）：合并权限，勿再分配帧。
            if vpn.start_addr().0 >= 0x8000_0000 {
                runtime::logging::trace!(
                    "[elf-load] PT_LOAD refuse overlap with kernel identity VPN={:#x}",
                    vpn.0
                );
            ...
            let merged = old | perm;
            aspace
                .protect_page(vpn, merged)
                .map_err(LoadElfError::Mm)?;
            runtime::logging::trace!(
                "[elf-load] PT_LOAD shared VPN={:#x} merge perm {:?}|{:?} -> {:?}",
                vpn.0,
                old,
                perm,
                merged
...
fn map_user_stack<A: AddressSpaceOps>(aspace: &mut A, stack_top: usize, stack_size: usize) -> Result<(), LoadElfError> {
    ...
    while vpn.0 < vpn_end.0 {
        let ppn = frame_alloc_result().map_err(|e| LoadElfError::Mm(MmError::from(e)))?;
        aspace
            .map_page_to_ppn(vpn, ppn, PagePerm::R | PagePerm::W | PagePerm::U)
            .map_err(LoadElfError::Mm)?;
        runtime::logging::info!(
            "[elf-load] stack alloc-map VPN={:#x} PPN={:#x} stack_top={:#x}",
            vpn.0,
            ppn.0,
            stack_top
        );
...
pub fn from_elf_bytes(data: &[u8]) -> Result<LoadedElf, LoadElfError> {
    ...
    if data.len() < 64 {
        runtime::logging::trace!("[elf-load] abort: TooSmall len={}", data.len());
        return Err(LoadElfError::TooSmall);
    }
    if &data[0..4] != b"\x7FELF" {
        runtime::logging::trace!("[elf-load] abort: BadMagic head={:02x?}", &data[..4]);
    ...
    if data.get(5) != Some(&1) {
        runtime::logging::trace!("[elf-load] abort: BadEndian ei_data={:?}", data.get(5));
        return Err(LoadElfError::BadEndian);
    }
    let e_machine = rd_u16(data, 18).ok_or(LoadElfError::TooSmall)?;
    if e_machine != EM_RISCV {
        runtime::logging::trace!(
            "[elf-load] abort: BadMachine e_machine={} (expect EM_RISCV={})",
            e_machine,
            EM_RISCV
        );
    ...
    if e_phentsize < 56 || e_phnum == 0 {
        runtime::logging::trace!(
            "[elf-load] abort: Parse bad ph e_phentsize={} e_phnum={}",
            e_phentsize,
            e_phnum
        );
        return Err(LoadElfError::Parse);
    ...
    runtime::logging::trace!(
        "[elf-load] ehdr e_entry={:#x} e_phoff={:#x} phentsize={} phnum={}",
        e_entry,
        e_phoff,
        e_phentsize,
        e_phnum
    ...
    for i in 0..e_phnum {
        ...
        if p_type != PT_LOAD {
            runtime::logging::trace!(
                "[elf-load] phdr i={} p_type={} (skip non-LOAD)",
                i,
                p_type
            );
            continue;
        ...
        runtime::logging::trace!(
            "[elf-load] PT_LOAD i={} vaddr={:#x} memsz={:#x} filesz={:#x} off={:#x} perm={:?}",
            i,
            p_vaddr,
            p_memsz,
            p_filesz,
    ...
    // 用户栈：固定顶与 16KiB 大小（均为 4K 页的整数倍）；与具体用户镜像链接脚本无关，属 bring-up 约定。
    const ELF_STACK_TOP: usize = 0x0000_0000_7fff_a000;
    const ELF_STACK_SIZE: usize = 16 * 1024;
    runtime::logging::trace!(
        "[elf-load] image range [{:#x},{:#x}) mapping done; map user stack top={:#x} size={}",
        min_vaddr,
        max_vaddr,
        ELF_STACK_TOP,
        ELF_STACK_SIZE
