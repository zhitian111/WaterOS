//! RISC-V **Sv39** 内存管理实现 crate：页表细节见内部 [`pagetable`]，对外仅通过
//! [`kernel_mm_impl`] 与自检入口暴露。
//!
...
pub fn test_with_range(start_ppn : PhysPageNum, end_ppn : PhysPageNum) {
    ...
    let mut aspace =
        pagetable::Sv39AddressSpace::new().expect("Sv39AddressSpace::new should succeed");
    let satp = aspace.satp_value();
    assert_eq!(satp >> 60,
               8,
               "satp mode should be Sv39");
...
/// Sv39 内核态用户拷贝自检；只操作临时地址空间，函数结束时释放页表和帧。
#[cfg(feature = "self_test")]
pub fn self_test() {
    log::info!("[mm-impl::sv39] self_test begin");
    user_access::test_copy_to_user_progress();
    log::info!("[mm-impl::sv39] self_test complete; temporary mappings reclaimed");
}

/// 内核全局页表与用户 ELF 装载（QEMU RISC-V bring-up）；由 `wateros-mm` 聚合为
/// `mm::kernel_mm`。
pub mod kernel_mm_impl {
    ...
    /// `UserTask::user_aspace_ptr()`， 即 `Sv39AddressSpace` 的泄漏裸指针。
    ///
