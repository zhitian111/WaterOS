...
unsafe impl Send for Sv39FileMapping {}

impl RegisteredFileMapping for Sv39FileMapping {
    ...
    fn invalidate(self : Box<Self>) {
        ...
        // A fork child can be registered before it is published as a handle.
        // It cannot run yet, so its construction-time PTE pointer is safe.
        let pte = unsafe { &mut *(self.pte as *mut Sv39Pte) };
...
impl Sv39AddressSpace {
    ...
    pub fn fork_cow(&mut self) -> MmResult<Sv39AddressSpace> {
        ...
        log::trace!("[mm-fork] Sv39AddressSpace::fork done child_root={}",
                    child_root.0);
        let mut child = Sv39AddressSpace { root : child_root,
                                           asid : child_asid,
        ...
        for (&vpn, &registration) in &self.shared_file_pages {
            let child_pte = child.walk_find(VirtPageNum(vpn))?
                                 .map(|(pte, _)| pte as *mut Sv39Pte as usize)
        ...
        where A : api_v0::frame_allocator::PhysicalFrameAllocator<FrameId = PhysPageNum>
    {
        ...
        if let DemandPage::SharedReadOnly(lease) = demand
        {
            ...
            let registration = match file_mapping::register(
                file_id,
                Box::new(Sv39FileMapping { pte : pte_context,
    ...
    /// Revoke one shared file PTE while holding this address space's lock.
    pub(crate) fn invalidate_file_page(&mut self,
        ...
        if !pte.flags().is_leaf_at_level(level) || pte.ppn() != expected_ppn {
            return false;
        }
        pte.clear();
