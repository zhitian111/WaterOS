...
use crate::{
    ...
    ProcessTaskDescriptor, ResourceLimit, SetResourceLimitError, TaskBlockReason, TaskClearTid,
    TaskId, TaskState, ThreadId,
};
...
pub fn find_exited_child_process_in_pgid(
...
pub fn stopped_child_ready_for_wait(
    parent_pid: ProcessId,
    child_pid: ProcessId,
) -> Option<ProcessDescriptor> {
...
pub fn find_stopped_child_process_in_pgid(
    parent_pid: ProcessId,
    pgid: ProcessId,
) -> Option<ProcessDescriptor> {
...
pub fn continued_child_ready_for_wait(
    parent_pid: ProcessId,
    child_pid: ProcessId,
) -> Option<ProcessDescriptor> {
...
pub fn find_continued_child_process_in_pgid(
    parent_pid: ProcessId,
    pgid: ProcessId,
) -> Option<ProcessDescriptor> {
    ...
    for task_id in task_ids {
            ...
            .is_some_and(|snapshot| matches!(snapshot.state, TaskState::Exited(_)))
        {
            if crate::schedule::current_task_id() == Some(task_id) {
                crate::schedule::block_current(TaskBlockReason::Manual);
            } else {
