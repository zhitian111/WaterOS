...
use crate::{
    active_impl, scheduler, ExitedTask, ProcessState, TaskId, TaskSnapshot,
    TaskTick, TaskWaitResult, TaskWaitTarget,
};
...
/// 让当前任务等待指定的阻塞对象。
pub fn wait_on(target: TaskWaitTarget) -> TaskWaitResult {
    scheduler::wait_current(target)
}

/// 在调度临界区内复查条件；条件仍成立才等待指定的阻塞对象。
pub fn wait_on_while(
...
pub fn wait_for_task_exit_for_ticks(task_id: TaskId, timeout_ticks: TaskTick) -> TaskWaitResult {
    wait_on_for_ticks(TaskWaitTarget::TaskExit(task_id), timeout_ticks)
}
...
pub fn interrupt_task(task_id: TaskId) -> bool {
    scheduler::interrupt_task(task_id)
}
