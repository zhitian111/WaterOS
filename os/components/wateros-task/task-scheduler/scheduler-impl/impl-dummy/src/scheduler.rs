...
impl RoundRobinScheduler {
    ...
    pub(super) fn allocate_wait_queue(&mut self) -> WaitQueueId { self.queues.allocate_wait_queue() }
    ...
    pub(super) fn current_task_id(&self) -> Option<TaskId> { self.registry.current_task_id() }
    ...
    pub(super) fn take_current_wait_result(&mut self) -> TaskWaitResult {
        self.registry.take_current_wait_result()
    }
}
