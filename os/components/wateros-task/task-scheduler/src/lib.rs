...
pub fn run_first_task() -> ! {
    active_impl::run_first_task()
}

/// 挂起当前任务并调度下一个可运行任务。
#[inline]
pub fn suspend_current_and_run_next() {
    active_impl::suspend_current_and_run_next();
}

/// 处理一次时钟 tick 相关的调度逻辑。
#[inline]
pub fn schedule_tick() {
    active_impl::schedule_tick();
}

/// 按给定原因阻塞当前任务。
#[inline]
pub fn block_current(reason: TaskBlockReason) {
    active_impl::block_current(reason);
}
...
pub fn exit_current(exit_code: TaskExitCode) -> ! {
    active_impl::exit_current(exit_code)
}

/// 返回当前正在运行任务的任务号。
#[inline]
pub fn current_task_id() -> Option<TaskId> {
    active_impl::current_task_id()
}
