...
impl Ext4Read for BlockDevRw {
    ...
    fn read(&self,
        ...
        read_with_small_cache(&self.device, start_byte, dst).map(|_| ())
...
// 本方法代码由AI完成
fn driver_boxed(e : DriverError) -> Box<dyn Error + Send + Sync + 'static> {
    Box::new(DriverErr(e))
}
...
impl ReadWriteFs for Ext4FsRw {
    ...
    fn write_regular_file(&mut self, path : &str, data : &[u8]) -> FsResult<()> {
        ...
        let parent_inode = fs.path_to_inode(parent_path, FollowSymlinks::All)
        ...
        let mut inode = fs.create_inode(InodeCreationOptions { file_type : FileType::Regular,
        ...
        parent_dir.link(name, &mut inode)
                  .map_err(map_ext4_plus)?;
        Ok(())
    }
    ...
    fn write_range(&mut self, path : &str, offset : u64, data : &[u8]) -> FsResult<usize> {
        ...
        while done < data.len() {
            let n = write_at(fs,
                             &mut inode,
                             &data[done..],
                             offset + done as u64).map_err(map_ext4_plus)?;
            if n == 0 {
    ...
    fn truncate(&mut self, path : &str, len : u64) -> FsResult<()> {
        ...
        let mut inode = fs.path_to_inode(pathv, FollowSymlinks::All)
                          .map_err(map_ext4_plus)?;
        if inode.file_type() != FileType::Regular {
            return Err(FsError::NotAFile);
        }
    ...
    fn mkdir(&mut self, path : &str, mode : u32) -> FsResult<()> {
        ...
        let mut inode = fs.create_inode(InodeCreationOptions { file_type : FileType::Directory,
                                                               mode : inode_mode,
                                                               uid : 0,
                                                               gid : 0,
                                                               time : Duration::from_secs(0),
    ...
    fn setxattr(&mut self, path : &str, name : &str, value : &[u8]) -> FsResult<()> {
        ...
        let mut inode = fs.path_to_inode(pathv, FollowSymlinks::All)
                          .map_err(map_ext4_plus)?;
        inode.set_xattr(fs, name.as_bytes(), value)
             .map_err(map_ext4_plus)?;
        Ok(())
    }
    ...
    fn getxattr(&self, path : &str, name : &str, buf : &mut [u8]) -> FsResult<usize> {
        ...
        let fs = self.fs()?;
        let pathv = Path::try_from(path).map_err(|_| FsError::InvalidPath)?;
        let inode = fs.path_to_inode(pathv, FollowSymlinks::All)
                      .map_err(map_ext4_plus)?;
        let value = inode.get_xattr(fs, name.as_bytes())
                         .map_err(map_ext4_plus)?
    ...
    fn listxattr(&self, path : &str, buf : &mut [u8]) -> FsResult<usize> {
        ...
        let inode = fs.path_to_inode(pathv, FollowSymlinks::All)
                      .map_err(map_ext4_plus)?;
        let names = inode.list_xattrs(fs)
                         .map_err(map_ext4_plus)?;
    ...
    fn removexattr(&mut self, path : &str, name : &str) -> FsResult<()> {
        ...
        let mut inode = fs.path_to_inode(pathv, FollowSymlinks::All)
                          .map_err(map_ext4_plus)?;
        inode.remove_xattr(fs, name.as_bytes())
             .map_err(map_ext4_plus)?;
        Ok(())
    }
    ...
    fn unlink(&mut self, path : &str) -> FsResult<()> {
        ...
        let mut parent_dir = Dir::open_inode(fs, parent_inode).map_err(map_ext4_plus)?;
        let name = DirEntryName::try_from(name).map_err(|_| FsError::InvalidPath)?;
        let target = parent_dir.get_entry(name)
                               .map_err(map_ext4_plus)?;
    ...
    fn rename(&mut self, old_path : &str, new_path : &str) -> FsResult<()> {
        ...
        let mut inode = parent_dir.get_entry(old_name)
                                  .map_err(map_ext4_plus)?;
        if let Ok(existing) = parent_dir.get_entry(new_name) {
            if existing.index == inode.index {
                return Ok(());
            }
            parent_dir.unlink(new_name, existing)
                      .map_err(map_ext4_plus)?;
        }
        parent_dir.link(new_name, &mut inode)
                  .map_err(map_ext4_plus)?;
        parent_dir.unlink(old_name, inode)
                  .map_err(map_ext4_plus)?;
        Ok(())
    }
    ...
    fn hardlink(&mut self, existing_path : &str, new_path : &str) -> FsResult<()> {
        ...
        let mut inode = fs.path_to_inode(existing_pathv, FollowSymlinks::All)
                          .map_err(map_ext4_plus)?;
        ...
        if parent_dir.get_entry(new_name)
                     .is_ok()
        {
            return Err(FsError::Exists);
        }
    ...
    fn metadata(&self, path : &str) -> FsResult<FsMetadata> {
        ...
        let inode = fs.path_to_inode(pathv, FollowSymlinks::All)
                      .map_err(map_ext4_plus)?;
        let meta = fs.metadata(pathv)
                     .map_err(map_ext4_plus)?;
        Ok(map_rw_metadata(&meta,
                           u64::from(inode.index
                                          .get())))
    }
    ...
    fn read(&self, path : &str) -> FsResult<Vec<u8>> {
        ...
        while filled < file_size {
            ...
            let n = file.read_bytes(&mut out[filled..filled + chunk])
                        .map_err(map_ext4_plus)?;
            if n == 0 {
                break;
            }
    ...
    fn read_range(&self, path : &str, offset : u64, buf : &mut [u8]) -> FsResult<usize> {
        ...
        while filled < to_read {
            ...
            let chunk = room.min(BLOCK_SIZE);
            let n = file.read_bytes(&mut buf[filled..filled + chunk])
                        .map_err(map_ext4_plus)?;
            if n == 0 {
                break;
            }
    ...
    fn read_dir(&self, path : &str) -> FsResult<Vec<FsDirEntry>> {
        ...
        while let Some(item) = rd.next() {
            ...
            out.push(FsDirEntry { name : String::from(name_str),
                                  node_type });
        }
        Ok(out)
    }
...
mod tests {
    ...
    impl CountingBlockDevice {
        fn shared(
            blocks : usize)
            -> (SharedBlockDevice, Arc<Mutex<Vec<u8>>>, Arc<Mutex<usize>>, Arc<Mutex<usize>>)
        {
            let bytes = Arc::new(Mutex::new(vec![0xAAu8; blocks * BLOCK_SIZE]));
            let reads = Arc::new(Mutex::new(0));
            let writes = Arc::new(Mutex::new(0));
            let dev = Self { bytes : bytes.clone(),
                             reads : reads.clone(),
                             writes : writes.clone() };
            (Arc::new(Mutex::new(Box::new(dev) as Box<dyn BlockDevice>)), bytes, reads, writes)
        }
    }
    ...
    impl BlockDevice for CountingBlockDevice {
        ...
        fn write_blocks(&mut self, start_block : Lba, buf : &[u8]) -> Result<(), DriverError> {
            ...
            *self.writes.lock() += 1;
            ...
            let dst = bytes.get_mut(start..end)
                           .ok_or(DriverError::InvalidParam)?;
            dst.copy_from_slice(buf);
            Ok(())
        }

        fn flush(&mut self) -> Result<(), DriverError> { Ok(()) }
    }
    ...
    fn aligned_full_blocks_write_without_read_modify_write() {
        ...
        assert_eq!(&bytes[..BLOCK_SIZE],
                   &[0xAAu8; BLOCK_SIZE]);
        assert_eq!(&bytes[BLOCK_SIZE..BLOCK_SIZE * 3],
                   src.as_slice());
        assert_eq!(&bytes[BLOCK_SIZE * 3..BLOCK_SIZE * 4],
                   &[0xAAu8; BLOCK_SIZE]);
    }
    ...
    fn unaligned_head_and_tail_preserve_uncovered_bytes() {
        ...
        let src : Vec<u8> = (0..len).map(|i| (i % 251) as u8)
                                    .collect();
        ...
        assert_eq!(&bytes[3 + len..BLOCK_SIZE * 3],
                   &[0xAAu8; BLOCK_SIZE - 7]);
    }
}
