...
fn random_uuid() -> Vec<u8> {
    ...
    format!("{:02x}{:02x}{:02x}{:02x}-{:02x}{:02x}-{:02x}{:02x}-{:02x}{:02x}-{:02x}{:02x}{:02x}{:\
             02x}{:02x}{:02x}\n",
            bytes[0],
            bytes[1],
            bytes[2],
...
impl ProcFsView for KernelProcFs {
    ...
    fn exists(&self, rel_path : &str) -> FsResult<bool> {
        ...
        Ok(match node {
            ...
            ProcNode::PidFd(pid, fd) => {
                task::leader_task_for_process(pid).map(|leader| fds_for(leader).contains(&fd))
                                                  .unwrap_or(false)
            }
    ...
    fn metadata(&self, rel_path : &str) -> FsResult<FsMetadata> {
        ...
        match node {
            ...
            ProcNode::PidNamespace(pid, _) => {
                if !process_visible(pid) {
                    return Err(FsError::NotFound);
                }
                Ok(FsMetadata { node_type : FsNodeType::Symlink,
                                size : self.read_symlink(rel_path)?
                                           .len() as u64,
                                mode : 0o777,
                                inode : proc_inode(node),
                                nlink : 1,
    ...
    fn read(&self, rel_path : &str) -> FsResult<Vec<u8>> {
        ...
        match node {
            ...
            ProcNode::PidRoot(_) |
            ProcNode::PidFd(_, _) |
            ProcNode::PidNamespace(_, _) => Err(FsError::NotAFile),
            ...
            // 只读兼容视图：当前不把 core image 写入文件系统；wait 的
            // WCOREDUMP/CLD_DUMPED 仍由信号默认动作与 RLIMIT_CORE 决定。
            ProcNode::SysKernelCorePattern => Ok(b"core\n".to_vec()),
            ...
            ProcNode::SysKernelRandomBootId => {
                Ok(b"57415445-524f-532d-424f-4f5449440001\n".to_vec())
            }
            ...
            ProcNode::PidMounts(pid) => process_visible(pid).then(format_mounts)
                                                            .ok_or(FsError::NotFound),
            ProcNode::PidMountinfo(pid) => format_mountinfo(pid),
            // cgroup controller/cgroupfs 尚未实现；空文件表示该进程未加入
            // 任何可见层级，比伪造 `0::/` 更准确。
            ProcNode::PidCgroup(pid) => process_visible(pid).then(Vec::new)
                                                            .ok_or(FsError::NotFound),
    ...
    fn read_range(&self, rel_path : &str, offset : u64, buf : &mut [u8]) -> FsResult<usize> {
        ...
        let static_data : &[u8] = match node {
            ProcNode::ProcNetTcp6 | ProcNode::ProcNetUdp6 => PROC_NET_TABLE,
            ProcNode::ProcNetRaw | ProcNode::ProcNetRaw6 => PROC_NET_TABLE,
            ProcNode::ProcNetUnix => PROC_NET_UNIX_TABLE,
            ProcNode::ProcNetTcp | ProcNode::ProcNetUdp => {
            ...
            ProcNode::SysKernelCorePattern => b"core\n",
    ...
    fn read_symlink(&self, rel_path : &str) -> FsResult<Vec<u8>> {
        ...
        match node {
            ProcNode::PidExe(pid) => {
                ...
                exe_for(process.leader_task_id).map(String::into_bytes)
                                               .ok_or(FsError::NotFound)
            }
            ProcNode::PidCwd(pid) => {
                let leader = task::leader_task_for_process(pid).ok_or(FsError::NotFound)?;
                cwd_for(leader).map(String::into_bytes)
                               .ok_or(FsError::NotFound)
            }
            ProcNode::PidRoot(pid) => {
                let leader = task::leader_task_for_process(pid).ok_or(FsError::NotFound)?;
                root_for(leader).map(String::into_bytes)
                                .ok_or(FsError::NotFound)
            }
            ProcNode::PidFd(pid, fd) => {
                let leader = task::leader_task_for_process(pid).ok_or(FsError::NotFound)?;
                ...
                Ok(fd_target_for(leader, fd).unwrap_or_else(|| {
                                                format!("anon_inode:[wateros-fd-{fd}]")
                                            })
                                            .into_bytes())
            }
            ProcNode::PidNamespace(pid, namespace) => {
                ...
                Ok(format!("{}:[{}]",
                           namespace.name(),
                           namespace.inode()).into_bytes())
            }
            ProcNode::SelfLink => {
                let pid = task::current_process_task_snapshot().ok_or(FsError::NotFound)?
                                                               .pid;
                Ok(pid.raw()
                      .to_string()
                      .into_bytes())
    ...
    fn read_dir(&self, rel_path : &str) -> FsResult<Vec<FsDirEntry>> {
        ...
        match node {
            ProcNode::Root => {
                let mut entries = vec![FsDirEntry { name : String::from("meminfo"),
                                       ...
                                       FsDirEntry { name : String::from("stat"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("loadavg"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("version"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("filesystems"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("devices"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("swaps"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("partitions"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("interrupts"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("cmdline"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("vmstat"),
                                                    node_type : FsNodeType::File },
                                       FsDirEntry { name : String::from("diskstats"),
                                                    node_type : FsNodeType::File },
                                       ...
                                       FsDirEntry { name : String::from("sys"),
                                                    node_type : FsNodeType::Directory },
                                       FsDirEntry { name : String::from("self"),
                                                    node_type : FsNodeType::Symlink },
                                       FsDirEntry { name : String::from("thread-self"),
                                                    node_type : FsNodeType::Symlink },];
            ...
            ProcNode::SysDir => Ok(vec![FsDirEntry { name:
                                                         String::from("kernel"),
                                                     node_type:
                                        ...
                                        FsDirEntry { name:
                                                     ...
                                                     node_type:
                                                         FsNodeType::Directory }]),
            ProcNode::SysKernelDir => Ok(vec![FsDirEntry { name:
                                                               String::from("pid_max"),
                                                           node_type:
                                                               FsNodeType::File },
                                              FsDirEntry { name:
                                              ...
                                              FsDirEntry { name:
                                                           ...
                                                           node_type:
                                                               FsNodeType::Directory },
                                              FsDirEntry { name:
                                                               String::from("randomize_va_space"),
                                                           node_type:
                                                               FsNodeType::File }]),
            ProcNode::SysKernelRandomDir => Ok(vec![FsDirEntry { name:
                                                                     String::from("boot_id"),
                                                                 node_type:
                                                                     FsNodeType::File },
                                                    FsDirEntry { name:
                                                                 ...
                                                                 node_type:
                                                                     FsNodeType::File },]),
            ProcNode::SysVmDir => Ok(vec![FsDirEntry { name:
                                          ...
                                          FsDirEntry { name:
                                                       ...
                                                       node_type:
                                                           FsNodeType::File }]),
            ProcNode::SysFsDir => Ok(vec![FsDirEntry { name:
                                          ...
                                          FsDirEntry { name:
                                                       ...
                                                       node_type:
                                                           FsNodeType::File },
                                          FsDirEntry { name:
                                                           String::from("aio-max-nr"),
                                                       node_type:
                                                           FsNodeType::File }]),
            ProcNode::SysNetDir => Ok(vec![FsDirEntry { name:
                                                            String::from("core"),
                                                        node_type:
                                                            FsNodeType::Directory },
                                           FsDirEntry { name:
                                                        ...
                                                        node_type:
                                                            FsNodeType::Directory },]),
            ProcNode::SysNetCoreDir => Ok(vec![FsDirEntry { name:
                                                                String::from("somaxconn"),
                                                            node_type:
                                                                FsNodeType::File }]),
            ProcNode::SysNetIpv4Dir => Ok(vec![FsDirEntry { name:
                                                                String::from("ip_local_port_range"),
                                                            node_type:
                                                                FsNodeType::File },
                                               FsDirEntry { name:
                                                            ...
                                                            node_type:
                                                                FsNodeType::File },]),
            ProcNode::NetDir => Ok(vec![FsDirEntry { name:
                                                         String::from("tcp"),
                                                     node_type:
                                                         FsNodeType::File },
                                        FsDirEntry { name:
                                        ...
                                        FsDirEntry { name:
                                                     ...
                                                     node_type:
                                                         FsNodeType::File },
                                        FsDirEntry { name:
                                                         String::from("sockstat"),
                                                     node_type:
                                                         FsNodeType::File },
                                        FsDirEntry { name:
                                                         String::from("sockstat6"),
                                                     node_type:
                                                         FsNodeType::File },]),
            ProcNode::PressureDir => Ok(vec![FsDirEntry { name:
                                                              String::from("cpu"),
                                                          node_type:
                                                              FsNodeType::File },
                                             FsDirEntry { name:
                                             ...
                                             FsDirEntry { name:
                                                          ...
                                                          node_type:
                                                              FsNodeType::File },]),
            ProcNode::SysVIpcDir => Ok(vec![FsDirEntry { name:
                                                             String::from("shm"),
                                                         node_type:
                                                             FsNodeType::File },
                                            FsDirEntry { name:
            ...
            ProcNode::PidNsDir(pid) => {
                ...
                Ok(ProcNamespace::ALL.into_iter()
                                     .map(|namespace| {
                                         FsDirEntry { name : String::from(namespace.name()),
                                                      node_type : FsNodeType::Symlink }
                                     })
                                     .collect())
            }
            ProcNode::PidTaskRoot(pid) => {
                ...
                Ok(task::task_ids_for_process(pid).unwrap_or_default()
                                                  .into_iter()
                                                  .map(|task_id| {
