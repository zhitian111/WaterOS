//! Socket 文件描述符句柄：将协议栈句柄桥接到 VFS [`VfsIoHandle`] trait。
use alloc::boxed::Box;
use alloc::sync::Arc;
use alloc::vec::Vec;
...
impl SocketRef {
    /// 包装协议栈句柄并分配唯一伪 inode。
    pub fn new(handle : StackSocketHandle) -> Self { Self::new_with_status_flags(handle, 0) }

    /// 包装协议栈句柄，并设置共享的打开状态标志（如 `O_NONBLOCK`）。
    pub fn new_with_status_flags(handle : StackSocketHandle, status_flags : usize) -> Self {
    ...
    /// 读取当前协议栈句柄（短暂持锁）。
    pub fn handle(&self) -> StackSocketHandle {
        *self.inner
             .handle
             .lock()
    }
    ...
    pub fn accept(&self) -> Result<(StackSocketHandle, [u8; 4], u16), stack::NetworkError> {
    ...
    pub fn prepare_receive(&self,
                           max_len : usize)
                           -> Result<SocketReceiveLease, stack::SocketRecvError> {
        let snapshot =
            stack::socket_poll_snapshot(self.handle()).map_err(|_| {
                                                          stack::SocketRecvError::InvalidSocket
                                                      })?;
...
impl SocketReceiveLease {
    ...
    pub fn finish(mut self,
        ...
        let reservation = self.reservation
                              .take()
                              .ok_or(stack::SocketRecvError::Io)?;
    ...
    fn poll_revents(&mut self, events : i16) -> VfsResult<i16> {
        ...
        if events & POLLIN != 0 {
            let read_ready = match snapshot.state {
                stack::SocketState::Listening { .. } => snapshot.has_pending_accept,
...
/// 将协议栈语义错误映射为 VFS 错误码。
fn map_stack_err(err : stack::NetworkError) -> VfsError {
    match err {
        stack::NetworkError::InvalidSocket |
        stack::NetworkError::WrongSocketType |
        stack::NetworkError::NotConnected => VfsError::BadFd,
        stack::NetworkError::Io |
        stack::NetworkError::Internal |
        stack::NetworkError::StackUnavailable => VfsError::Io,
