...
use crate::{
    NetworkAddress, NetworkEndpoint, NetworkResult, SocketDomain, SocketKind, SocketPollSnapshot,
    SocketSendError,
};
...
impl SocketRef {
    ...
    pub fn new_tcp(domain : SocketDomain, status_flags : usize) -> NetworkResult<Self> {
        let handle = stack::create_tcp_socket(domain)?;
        Ok(Self::from_stack_handle(handle,
                                   domain,
                                   status_flags))
    }

    /// 创建 UDP socket，并把底层句柄封装进共享生命周期对象。
    pub fn new_udp(domain : SocketDomain, status_flags : usize) -> NetworkResult<Self> {
        let handle = stack::create_udp_socket(domain)?;
        Ok(Self::from_stack_handle(handle,
                                   domain,
                                   status_flags))
    }

    /// 创建仅允许 Echo Request/Reply 的 raw ICMP/ICMPv6 socket。
    pub fn new_icmp(domain : SocketDomain, status_flags : usize) -> NetworkResult<Self> {
        let handle = stack::create_icmp_socket(domain)?;
        Ok(Self::from_stack_handle(handle, domain, status_flags))
    }

    /// 仅供本模块包装新建或 accept 得到的底层句柄。
    fn from_stack_handle(handle : StackSocketHandle,
                         domain : SocketDomain,
                         status_flags : usize)
                         -> Self {
    ...
    pub fn accept(&self, status_flags : usize) -> NetworkResult<(SocketRef, NetworkEndpoint)> {
        let mut listener = self.inner
                               .handle
                               .lock();
        let (established, replacement, peer) = stack::socket_accept(*listener)?;
        *listener = replacement;
        Ok((Self::from_stack_handle(established, self.domain(), status_flags), peer))
    }

    pub fn kind(&self) -> NetworkResult<SocketKind> { self.with_handle(stack::socket_kind) }

    pub fn domain(&self) -> SocketDomain { self.inner.domain }

    pub fn bind(&self, local_ip : Option<NetworkAddress>, port : u16) -> NetworkResult<()> {
        self.with_handle(|handle| stack::socket_bind(handle, local_ip, port))
    }

    pub fn connect(&self, endpoint : NetworkEndpoint) -> NetworkResult<()> {
        self.with_handle(|handle| stack::socket_connect(handle, endpoint.address, endpoint.port))
    }
    ...
    pub fn local_endpoint(&self) -> NetworkResult<NetworkEndpoint> {
        self.with_handle(stack::socket_local_endpoint)
    }

    pub fn peer_endpoint(&self) -> NetworkResult<NetworkEndpoint> {
    ...
    pub fn ipv6_pktinfo_type(&self) -> NetworkResult<Option<i32>> {
        self.with_handle(stack::socket_ipv6_pktinfo_type)
    }
