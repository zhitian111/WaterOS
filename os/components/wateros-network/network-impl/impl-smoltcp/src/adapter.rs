...
impl SmoltcpAdapter {
    ...
    fn drain_rx_staging(&mut self) {
        while self.rx_staging
                  .len() <
              MAX_RX_DRAIN
        {
            if let Some(frame) = self.loopback_queue
                                     .pop_front()
            {
                self.rx_staging
                    .push_back(frame);
...
impl Device for SmoltcpAdapter {
    type RxToken<'a>
        = SmoltcpRxToken<'a> where Self : 'a;
    type TxToken<'a>
        = SmoltcpTxToken<'a> where Self : 'a;
    ...
    fn consume<R, F : FnOnce(&mut [u8]) -> R>(self, len : usize, f : F) -> R {
        ...
        if let Some(dev) = self.dev {
            if let Err(e) = dev.lock()
                               .send(frame)
            {
                log::warn!("[smoltcp-adapter] tx send failed: {:?}",
                           e);
            }
        }
        result
