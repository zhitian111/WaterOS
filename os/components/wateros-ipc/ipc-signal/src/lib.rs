...
impl IntervalTimerState {
    ...
    fn replace(&mut self, spec: IntervalTimerSpec, now_ns: u128) {
        ...
        self.deadline_ns =
            (spec.value_ns != 0).then(|| now_ns.saturating_add(spec.value_ns));
...
impl SignalRegistry {
    ...
    pub fn has_process(&self, pid: usize) -> bool { self.processes.contains_key(&pid) }

    pub fn has_thread(&self, task_id: usize) -> bool { self.threads.contains_key(&task_id) }
    ...
    pub fn fork_process(
        ...
        self.threads.insert(child_task_id,
                            ThreadSignalState::new(child_pid,
                                                   child_tid,
                                                   parent_thread.mask));
        Ok(())
    }
    ...
    pub fn register_thread(
        ...
        let parent = *self.threads
                          ...
                          .ok_or(SignalError::NoSuchTask)?;
        self.threads
            .insert(task_id, ThreadSignalState::new(parent.pid, tid, parent.mask));
        Ok(())
    }
    ...
    pub fn drop_process(&mut self, pid: usize) {
        ...
        self.threads.retain(|_, thread| thread.pid != pid);
    }
    ...
    pub fn set_action(
        ...
        if action.is_ignore() {
            process.pending.remove(sig);
            for thread in self.threads.values_mut().filter(|thread| thread.pid == pid) {
                thread.pending.remove(sig);
            }
        }
        Ok(old)
    }
    ...
    pub fn current_mask(&self, task_id: usize) -> SignalResult<SignalSet> {
        Ok(self.thread(task_id)?.mask)
    }

    pub fn replace_mask(&mut self, task_id: usize, mut mask: SignalSet) -> SignalResult<()> {
        mask.remove(SIGKILL);
        mask.remove(SIGSTOP);
        self.thread_mut(task_id)?.mask = mask;
        Ok(())
    }

    pub fn begin_sigsuspend(
        &mut self,
        task_id: usize,
        ...
        if thread.suspend_restore_mask.is_some() {
    ...
    pub fn begin_signal_wait(
        &mut self,
        task_id: usize,
        wait_set: SignalSet,
    ) -> SignalResult<()> {
        self.thread_mut(task_id)?.waiting_for = Some(wait_set);
        Ok(())
    }

    pub fn end_signal_wait(&mut self, task_id: usize) -> SignalResult<()> {
        self.thread_mut(task_id)?.waiting_for = None;
        Ok(())
    }
    ...
    pub fn update_mask(
        ...
        thread.mask = match how {
            SIG_BLOCK => thread.mask.union(set),
            SIG_UNBLOCK => thread.mask.difference(set),
            SIG_SETMASK => set,
    ...
    pub fn send_thread(&mut self, task_id: usize, sig: usize) -> SignalResult<SignalDispatch> {
        ...
        match self.classify(pid, sig)? {
            ...
            SignalDelivery::Pending => {
                self.thread_mut(task_id)?.pending.insert(sig);
    ...
    pub fn send(&mut self, task_id: usize, sig: usize) -> SignalResult<SignalDelivery> {
        Ok(self.send_thread(task_id, sig)?.delivery)
    }
    ...
    pub fn pending(&self, task_id: usize) -> SignalResult<SignalSet> {
        ...
        let process = self.processes
                          .get(&thread.pid)
                          .ok_or(SignalError::NoSuchProcess)?;
        Ok(thread.pending.union(process.pending))
    }
    ...
    pub fn take_pending(&mut self, task_id: usize, wait_set: SignalSet) -> Option<usize> {
        let thread = *self.threads.get(&task_id)?;
        let thread_ready = thread.pending.intersection(wait_set);
        if let Some(sig) = thread_ready.first_signal() {
            self.threads.get_mut(&task_id)?.pending.remove(sig);
            return Some(sig);
        }
        let process = self.processes.get_mut(&thread.pid)?;
        let sig = process.pending.intersection(wait_set).first_signal()?;
        process.pending.remove(sig);
        Some(sig)
    }
    ...
    pub fn take_deliverable(&mut self, task_id: usize) -> Option<PendingSignal> {
        let thread = *self.threads.get(&task_id)?;
        let process = self.processes.get(&thread.pid)?;
        ...
        if self.threads
               .get(&task_id)
               .is_some_and(|thread| thread.pending.contains(sig))
        {
            self.threads.get_mut(&task_id)?.pending.remove(sig);
        } else {
            self.processes.get_mut(&thread.pid)?.pending.remove(sig);
        }
        let previous_mask = thread.suspend_restore_mask.unwrap_or(thread.mask);
        let mut handler_mask = previous_mask.union(action.mask);
        if action.flags & SA_NODEFER == 0 {
            handler_mask.insert(sig);
        }
        let target_thread = self.threads.get_mut(&task_id)?;
        target_thread.mask = handler_mask;
        target_thread.suspend_restore_mask = None;
        target_thread.waiting_for = None;
        if action.flags & SA_RESETHAND != 0 {
            self.processes.get_mut(&thread.pid)?.actions[sig] =
    ...
    pub fn set_timer(
        ...
        let old = process.timer(which)?.remaining(now);
    ...
    pub fn get_timer(
        ...
        let process = self.processes
                          ...
                          .ok_or(SignalError::NoSuchProcess)?;
        let now = process.timer_clock(which, monotonic_ns)?;
        Ok(process.timer(which)?.remaining(now))
    }
    ...
    pub fn expire_realtime(&mut self, monotonic_ns: u128) -> Vec<SignalDispatch> {
        ...
        for deadline in deadlines {
            let entries = self.real_deadlines.remove(&deadline).unwrap_or_default();
            for entry in entries {
                let Some(process) = self.processes.get_mut(&entry.pid) else {
                    continue;
                };
                if process.real.generation != entry.generation ||
                   process.real.deadline_ns != Some(deadline)
                {
                    continue;
                }
                if !process.real.expire(monotonic_ns) {
                    continue;
                }
                let next = process.real.deadline_ns;
                let generation = process.real.generation;
...
mod tests {
    ...
    fn realtime_timer_replaces_disables_and_advances_without_drift() {
        ...
        registry.set_action(100,
                ...
                .unwrap();
        let first = IntervalTimerSpec {
            interval_ns: 10,
            value_ns: 20,
        };
        assert_eq!(registry.set_timer(10, ITIMER_REAL, first, 100).unwrap(),
                   IntervalTimerSpec::default());
        assert_eq!(registry.get_timer(10, ITIMER_REAL, 105).unwrap().value_ns, 15);

        let dispatches = registry.expire_realtime(145);
        assert_eq!(dispatches.len(), 1);
        assert_eq!(registry.get_timer(10, ITIMER_REAL, 145).unwrap().value_ns, 5);
        assert_eq!(registry.pending(100).unwrap().bits(),
                   SignalSet::from_bits(1 << (SIGALRM - 1)).bits());

        let old = registry
            .set_timer(10, ITIMER_REAL, IntervalTimerSpec::default(), 146)
            .unwrap();
        assert_eq!(old.value_ns, 4);
        assert_eq!(registry.get_timer(10, ITIMER_REAL, 1_000).unwrap().value_ns, 0);
    }

    #[test]
    fn standard_pending_coalesces_and_targets_lowest_unmasked_tid() {
        let mut registry = registry_with_process();
        registry.register_thread(100, 101, 11).unwrap();
        registry
            .replace_mask(100, SignalSet::from_bits(1 << (SIGALRM - 1)))
            .unwrap();
        let action = SignalAction {
            handler: 0x1000,
            ..SignalAction::default_action()
        };
        registry.set_action(100, SIGALRM, action).unwrap();

        let first = registry.send_process(10, SIGALRM).unwrap();
        let second = registry.send_process(10, SIGALRM).unwrap();
        assert_eq!(first.target_task_id, Some(101));
        assert_eq!(second.target_task_id, Some(101));
        assert_eq!(registry.pending(101).unwrap().bits().count_ones(), 1);
    }
    ...
    fn fork_copies_dispositions_and_mask_but_not_timer_or_pending() {
        ...
        registry.set_action(100, SIGUSR1, action).unwrap();
        registry
            .replace_mask(100, SignalSet::from_bits(1 << (SIGUSR2 - 1)))
            .unwrap();
        registry
            .set_timer(10,
                       ITIMER_REAL,
                       ...
                       100)
            .unwrap();
        registry.send_process(10, SIGUSR1).unwrap();

        registry.fork_process(100, 20, 200, 20).unwrap();
        assert_eq!(registry.get_action(200, SIGUSR1).unwrap(), action);
        assert!(registry.current_mask(200).unwrap().contains(SIGUSR2));
        assert_eq!(registry.get_timer(20, ITIMER_REAL, 100).unwrap(),
                   IntervalTimerSpec::default());
        assert!(registry.pending(200).unwrap().is_empty());
    }

    #[test]
    fn cpu_timers_use_distinct_user_and_total_clocks() {
        let mut registry = registry_with_process();
        for (signal, handler) in [(SIGVTALRM, 0x3000), (SIGPROF, 0x4000)] {
            registry.set_action(100,
                                signal,
                                SignalAction {
                                    handler,
        ...
        assert!(registry.account_cpu(10, 9, 9).unwrap().is_empty());
        let expired = registry.account_cpu(10, 1, 6).unwrap();
        assert_eq!(expired.len(), 2);
        assert!(registry.pending(100).unwrap().contains(SIGVTALRM));
        assert!(registry.pending(100).unwrap().contains(SIGPROF));
    }
    ...
    fn realtime_generation_discards_stale_deadline_entries() {
        ...
        assert!(registry.expire_realtime(120).is_empty());
        assert_eq!(registry.expire_realtime(140).len(), 1);
    }

    #[test]
    fn exec_preserves_ignore_pending_and_timer_but_resets_caught_handler() {
        let mut registry = registry_with_process();
        registry.set_action(100, SIGUSR1, SignalAction::ignore()).unwrap();
        registry.set_action(100,
                            SIGUSR2,
                            SignalAction {
                                handler: 0x6000,
        ...
        registry.set_timer(10,
                           ...
                           100)
                .unwrap();
        registry.send_thread(100, SIGUSR2).unwrap();

        registry.exec_process(100).unwrap();

        assert!(registry.get_action(100, SIGUSR1).unwrap().is_ignore());
        assert!(registry.get_action(100, SIGUSR2).unwrap().is_default());
        assert!(registry.pending(100).unwrap().contains(SIGUSR2));
        assert_eq!(registry.get_timer(10, ITIMER_REAL, 105).unwrap().value_ns, 15);
    }
    ...
    fn sigsuspend_restores_original_mask_through_signal_frame() {
        ...
        registry.replace_mask(100, original).unwrap();
        registry.begin_sigsuspend(100, SignalSet::empty()).unwrap();
        registry.send_thread(100, SIGUSR1).unwrap();

        let pending = registry.take_deliverable(100).unwrap();
        assert_eq!(pending.previous_mask, original);
        registry.restore_mask(100, pending.previous_mask).unwrap();
        assert_eq!(registry.current_mask(100).unwrap(), original);
    }
}
