...
//!
//! `ARCH:` 类型别名与 `wateros-task` 调度子系统对齐；IPC 侧仅定义薄包装 trait，不引入第二套
//! 等待语义。任务状态、timeout、ready CPU 和 IPI 均由 task scheduler 所有。
...
///
/// `LOCK:` 调用条件等待前不能持有会被条件闭包或唤醒路径再次取得的 IPC 对象锁。条件闭包在
/// scheduler 临界区执行，因此只应做短小、非阻塞的状态检查。
pub trait IpcWaitQueueOps: Sized {
    /// 创建新的等待队列。
    fn new() -> Self;

    /// 创建带静态诊断名的队列。实现不支持命名时可退化为 [`Self::new`]。
    ...
    /// 队列当前没有 waiter 时释放其底层 ID；仍有外部并发引用时不得调用。
    ...
    /// 原子地唤醒前若干 waiter，再迁移后续 waiter 到目标队列。
    fn requeue_to(&self, target: Self, wake_count: usize, requeue_count: usize) -> usize;

    /// 在 scheduler 临界区复查条件后执行 [`Self::requeue_to`]；条件不成立返回 `None`。
