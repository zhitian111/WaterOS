...
impl EpollHandle {
    ...
    pub(crate) fn instance(&self) -> Arc<Mutex<EpollInstance>> {
        self.inner.clone()
    }
}
...
impl VfsIoHandle for EpollHandle {
    ...
    fn close(&mut self) -> VfsResult<()> {
        self.inner.lock().interests.clear();
        Ok(())
    }

    fn metadata(&self) -> VfsResult<VfsMetadata> {
        let inode = self.inner.lock().inode;
        Ok(VfsMetadata {
...
impl EpollFdRegistry {
    ...
    fn register(&mut self, task_id: task::TaskId, fd: usize, instance: Arc<Mutex<EpollInstance>>) {
...
static EPOLL_FD_REGISTRY: Mutex<EpollFdRegistry> = Mutex::new(EpollFdRegistry {
    maps: BTreeMap::new(),
    owners: BTreeMap::new(),
...
pub(crate) fn register(fd: usize, instance: Arc<Mutex<EpollInstance>>) {
    if let Some(task_id) = task::current_task_id() {
        EPOLL_FD_REGISTRY.lock().register(task_id, fd, instance);
    }
}

pub(crate) fn lookup(fd: usize) -> Option<Arc<Mutex<EpollInstance>>> {
    let task_id = task::current_task_id()?;
    EPOLL_FD_REGISTRY.lock().lookup(task_id, fd)
}

pub(crate) fn is_epoll_fd(fd: usize) -> bool {
    lookup(fd).is_some()
}

pub(crate) fn remove(fd: usize) {
    if let Some(task_id) = task::current_task_id() {
        EPOLL_FD_REGISTRY.lock().remove(task_id, fd);
    }
}
...
pub(crate) fn drop_task(task_id: task::TaskId) {
    EPOLL_FD_REGISTRY.lock().release_task(task_id);
}
...
pub(crate) const EPOLL_VALID_EVENTS: u32 = EPOLLIN
    | EPOLLOUT
    | EPOLLPRI
    | EPOLLERR
    | EPOLLHUP
