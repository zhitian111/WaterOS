...
use crate::user_copy::{copy_from_user, copy_to_user, copy_to_user_progress, copy_to_user_struct};
...
impl DgramInbox {
    fn new() -> Self {
        Self { state : Mutex::new(DgramInboxState { queue : VecDeque::new(),
    ...
    fn push(&self, packet : DgramPacket) -> Result<(), ErrNo> {
        ...
        let occupied = state.queue.len() +
                       usize::from(state.active_reservation
                                        .is_some());
    ...
    fn acquire(self: &Arc<Self>, nonblocking : bool) -> Result<DgramReadLease, VfsError> {
        loop {
            ...
            if state.active_reservation
                    .is_none()
            {
                if let Some(packet) = state.queue
                                           .pop_front()
                {
                    let id = state.next_reservation_id;
                    state.next_reservation_id = state.next_reservation_id
                                                     .wrapping_add(1)
                                                     .max(1);
                    state.active_reservation = Some(id);
                    drop(state);
                    return Ok(DgramReadLease { inbox : self.clone(),
...
impl DgramReadLease {
    ...
    fn finish(mut self, copied : usize, complete : bool) -> VfsResult<VfsReadFinish> {
...
impl Drop for DgramReadLease {
    fn drop(&mut self) {
        let (Some(id), Some(packet)) = (self.reservation_id
                                            .take(),
                                        self.packet.take())
        else {
            return;
...
pub(crate) fn unregister(task_id : usize, fd : usize) {
    ...
    let still_referenced = table.values()
...
pub(crate) fn copy_fds_from_parent(child : usize, parent : usize) {
    let mut table = FD_TABLE.lock();
    let inherited : Vec<_> = table.range((parent, 0)..=(parent, usize::MAX))
                                  .map(|(&(_, fd), sock)| ((child, fd), sock.clone()))
                                  .collect();
...
pub(crate) fn drop_task(task_id : usize) {
    let fds : Vec<usize> = FD_TABLE.lock()
...
pub(crate) fn alloc_unix_socket(typ : usize,
                                status_flags : usize)
    ...
    let dgram_inbox = (sock_type == UnixSockType::Dgram).then(|| Arc::new(DgramInbox::new()));
    let sock = UnixSockRef { inner:
                                 Arc::new(Mutex::new(UnixSockInner { sock_type,
                                                                     nonblocking,
                                                                     bound_key : None,
                                                                     peer_key : None,
                                                                     listening : false,
    ...
    let handle = Box::new(UnixSocketHandle { sock : sock.clone(),
                                             inode });
    Ok((handle, sock))
}
...
pub(crate) fn alloc_unix_stream_pair(
    nonblocking : bool)
    -> ((Box<dyn VfsIoHandle>, UnixSockRef), (Box<dyn VfsIoHandle>, UnixSockRef)) {
            ...
            let sock =
                UnixSockRef { inner:
                                  Arc::new(Mutex::new(UnixSockInner { sock_type:
                                                                      ...
                                                                      dgram_inbox : None })) };
            let inode = NEXT_INODE.fetch_add(1, Ordering::Relaxed);
            let handle : Box<dyn VfsIoHandle> = Box::new(UnixSocketHandle { sock : sock.clone(),
                                                                            inode });
            (handle, sock)
        };
...
pub(crate) fn parse_sockaddr_un(addr_ptr : usize, addrlen : usize) -> Result<UnixAddr, ErrNo> {
    ...
    if path_len == 0 {
        return Ok(UnixAddr { key : Vec::new(),
                             abstract_ns : false });
    }
...
pub(crate) fn bind(fd : usize, addr_ptr : usize, addrlen : usize) -> Result<(), ErrNo> {
    ...
    let dgram_inbox = inner.dgram_inbox
                           .clone();
    bound.insert(addr.key.clone(),
                 BoundEntry { sock_type : inner.sock_type,
                              listening : false,
...
pub(crate) fn getsockname(fd : usize, addr_ptr : usize, addrlen_ptr : usize) -> Result<(), ErrNo> {
    let sock = lookup_current(fd)?;
    let key = sock.inner
                  .lock()
                  .bound_key
...
pub(crate) fn getpeername(fd : usize, addr_ptr : usize, addrlen_ptr : usize) -> Result<(), ErrNo> {
    let sock = lookup_current(fd)?;
    let inner = sock.inner.lock();
    let key = inner.peer_key
                   .clone()
                   .or_else(|| {
                       inner.dgram_peer
                            .clone()
            ...
            let accepted =
                UnixSockRef { inner:
                                  Arc::new(Mutex::new(UnixSockInner { sock_type:
                                                                      ...
                                                                      dgram_inbox : None })) };
            let inode = NEXT_INODE.fetch_add(1, Ordering::Relaxed);
            return Ok((Box::new(UnixSocketHandle { sock : accepted.clone(),
                                                   inode }),
                       accepted));
        }
        let nonblocking = sock.inner
                              .lock()
                              .nonblocking;
        if nonblocking {
            return Err(ErrNo::EAGAIN);
...
pub(crate) fn sendto_unix(fd : usize,
                          buf : &[u8],
                          addr_ptr : usize,
                          addrlen : usize)
                          -> Result<usize, ErrNo> {
    let sock = lookup_current(fd)?;
    let inner = sock.inner.lock();
    if let Some(mut endpoint) = inner.endpoint
                                     .clone()
    {
        drop(inner);
    ...
    let sender_key = inner.bound_key
                          .clone();
    if addr_ptr == 0 && addrlen == 0 {
        if let Some(peer) = inner.dgram_peer_inbox
                                 .clone()
        {
            drop(inner);
    ...
    let inbox = entry.dgram_inbox
                     .clone()
                     .ok_or(ErrNo::ECONNREFUSED)?;
    drop(bound);
    inbox.push(DgramPacket { data : buf.to_vec(),
                             sender : sender_key })?;
    Ok(buf.len())
}

pub(crate) fn recvfrom_unix(fd : usize,
                            buf_ptr : usize,
                            max_len : usize,
                            addr_ptr : usize,
                            addrlen_ptr : usize)
...
fn write_unix_addr_to_user(addr_ptr : usize,
                           addrlen_ptr : usize,
                           key : &[u8])
                           -> Result<(), ErrNo> {
    ...
    // （getpeername01 / getsockname01 以 -1 期望 EINVAL）。
    if addrlen as usize > i32::MAX as usize {
        return Err(ErrNo::EINVAL);
    }
    let write_len = addr.len()
                        .min(addrlen as usize);
...
fn lookup_current(fd : usize) -> Result<UnixSockRef, ErrNo> {
    ...
    if let Some(sock) = FD_TABLE.lock()
                                .get(&(task_id, fd))
                                .cloned()
    {
        return Ok(sock);
...
impl VfsIoHandle for UnixSocketHandle {
    ...
    fn prepare_read(&mut self, max_len : usize) -> VfsResult<Box<dyn VfsPreparedRead>> {
        ...
        let inbox = inner.dgram_inbox
                         .clone()
                         .ok_or(VfsError::Unsupported)?;
        Ok(Box::new(DgramPreparedRead { inbox,
    ...
    fn set_open_status_flags(&mut self, flags : u32) -> VfsResult<()> {
        ...
        if let Some(endpoint) = inner.endpoint
                                     .as_mut()
        {
            endpoint.set_open_status_flags(if nonblocking { SOCK_NONBLOCK as u32 } else { 0 })?;
        }
        Ok(())
    }
    ...
    fn read(&mut self, buf : &mut [u8]) -> VfsResult<usize> {
        ...
        match lease.finish(VfsCopyProgress { copied : n,
                                             complete : true })?
        {
            VfsReadFinish::Bytes(copied) => Ok(copied),
    ...
    fn poll_revents(&mut self, events : i16) -> VfsResult<i16> {
        ...
        if events & POLLIN != 0 {
            ...
            let has_data = inner.dgram_inbox
                                .as_ref()
                                .is_some_and(|inbox| inbox.has_data());
            drop(inner);
            let has_pending_accept = listening_key.as_ref()
                                                  .is_some_and(|key| {
                                                      BOUND.lock()
                                                           .get(key)
                                                           .is_some_and(|entry| {
    ...
    fn poll_wait_for_ticks(&mut self,
        ...
        let inner = self.sock
                        .inner
                        .lock();
        let Some(mut endpoint) = inner.endpoint
                                      .clone()
        else {
    ...
    fn duplicate(&self) -> VfsResult<Box<dyn VfsIoHandle>> {
        Ok(Box::new(Self { sock : self.sock
                                      .clone(),
                           inode : self.inode }))
    }
}
...
impl VfsPreparedRead for DgramPreparedRead {
    fn acquire(self: Box<Self>) -> VfsResult<Box<dyn VfsReadLease>> {
        let lease = self.inbox
                        .acquire(self.nonblocking)?;
        Ok(Box::new(DgramVfsReadLease { lease : Some(lease),
