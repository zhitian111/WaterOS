...
fn resolve_readlink_prefix_symlinks(path : &str) -> Result<String, ErrNo> {
    ...
    'follow: for _ in 0..40 {
        let parts : alloc::vec::Vec<String> = current.trim_start_matches('/')
                                                     .split('/')
        ...
        for index in 0..parts.len() - 1 {
            ...
            match active_impl::backend().metadata(prefix.as_str()) {
                ...
                Ok(meta) if meta.node_type == VfsNodeType::Symlink => {
                    let target =
                        vfs::read_symlink_absolute(prefix.as_str()).map_err(vfs_error_to_errno)?;
                    let target =
                        core::str::from_utf8(target.as_slice()).map_err(|_| ErrNo::EINVAL)?;
                    let parent =
                        prefix.rsplit_once('/')
                              .map(|(parent, _)| if parent.is_empty() { "/" } else { parent })
                              .unwrap_or("/");
                    let mut next =
                        resolve_against_cwd(parent, Some(target)).map_err(vfs_error_to_errno)?;
...
fn read_current_exe(buf_ptr : usize, bufsiz : usize) -> UserRet {
    ...
    match copy_to_user(buf_ptr, &bytes[..write_len]) {
        ...
        Ok(n) => {
            log::trace!("[readlinkat] /proc/self/exe partial copy n={n} expected={write_len} \
                         buf={buf_ptr:#x} path={exe_path:?}");
            UserRet::from_error(ErrNo::EFAULT)
        }
        Err(e) => {
            log::trace!("[readlinkat] /proc/self/exe copy failed errno={e:?} buf={buf_ptr:#x} \
