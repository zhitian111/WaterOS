...
use vfs::active_impl;
use vfs::api::{FinalSymlink, SingleRootReadView, VfsError, VfsMetadata, VfsNodeType};
...
fn rename_at_impl(old_dirfd : isize,
                  old_path_ptr : usize,
                  new_dirfd : isize,
                  new_path_ptr : usize,
                  flags : u32)
    ...
    // Linux rename 语义：oldpath / newpath 的中间符号链接都要展开（如
    // usrmerge 的 `/bin -> usr/bin`），最终分量不跟随（old 移动链接本身，
    // new 是覆盖目标）。此前漏掉会与 stat 的解析不一致（EISDIR/ENOENT
    // 矛盾），且在后端对字面 `/bin/...` 操作失败。
    let old_resolved = match resolve_symlinks(old_resolved.as_str(),
    ...
    if let Err(e) = check_existing_type_mismatch(old_resolved.as_str(),
                                                 new_resolved.as_str())
    {
    ...
    if flags & RENAME_EXCHANGE != 0 {
        if let Err(e) = check_rename_permission(old_resolved.as_str(),
                                                new_resolved.as_str(),
                                                true)
        {
            return UserRet::from_error(e);
        ...
        let new_is_dir = active_impl::backend().metadata(new_resolved.as_str())
                                               .map(|meta| meta.node_type == VfsNodeType::Directory)
                                               .unwrap_or(false);
        return match rename_exchange(old_resolved.as_str(),
                                     new_resolved.as_str())
        {
            Ok(()) => {
    ...
    match vfs::rename_absolute(old_resolved.as_str(),
                               new_resolved.as_str())
    {
        Ok(()) => {
            super::inotify::notify_move(old_resolved.as_str(),
...
fn check_existing_type_mismatch(old_path : &str, new_path : &str) -> Result<(), ErrNo> {
    let old_meta = active_impl::backend().metadata(old_path)
...
fn check_parent_write_search(path : &str,
...
fn check_sticky_parent(parent_meta : &VfsMetadata,
...
fn rename_exchange(old_path : &str, new_path : &str) -> Result<(), ErrNo> {
    active_impl::backend().metadata(old_path)
                          .map_err(vfs_error_to_errno)?;
    active_impl::backend().metadata(new_path)
                          .map_err(vfs_error_to_errno)?;
...
fn exchange_temp_path(old_path : &str) -> alloc::string::String {
    let parent = old_path.rsplit_once('/')
                         .map(|(parent, _)| if parent.is_empty() { "/" } else { parent })
                         .unwrap_or("/");
