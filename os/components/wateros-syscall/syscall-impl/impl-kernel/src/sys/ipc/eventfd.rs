...
impl EventFdHandle {
    fn new(counter: u64, nonblocking: bool, semaphore: bool) -> Self {
        Self {
            state: Arc::new(EventFdState::new(counter, nonblocking)),
            semaphore,
        }
    }
}
...
impl VfsIoHandle for EventFdHandle {
    fn read(&mut self, buf: &mut [u8]) -> VfsResult<usize> {
        ...
        loop {
            ...
            if self
                .state
                .wait
                .wait_current_while(|| self.state.inner.lock().counter == 0)
    ...
    fn write(&mut self, buf: &[u8]) -> VfsResult<usize> {
        ...
        let value = u64::from_ne_bytes(buf[..8].try_into().expect("eventfd write width"));
        if value == u64::MAX {
            return Err(VfsError::InvalidPath);
        }
        loop {
            ...
            if self
                .state
                .wait
                .wait_current_while(|| self.state.inner.lock().counter > MAX_COUNTER - value)
                == task::TaskWaitResult::Interrupted
            {
                return Err(VfsError::Interrupted);
            }
    ...
    fn poll_wait_for_ticks(
        ...
        let result = self.state.wait.wait_current_while_for_ticks(timeout_ticks, || {
            still_waiting() && self.state.ready(events) == 0
        });
    ...
    fn set_open_status_flags(&mut self, flags: u32) -> VfsResult<()> {
        self.state.inner.lock().nonblocking = flags & EFD_NONBLOCK as u32 != 0;
        Ok(())
    }
}
