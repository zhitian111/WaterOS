...
/// Linux `struct seminfo`（semctl IPC_INFO/SEM_INFO 输出）。
#[repr(C)]
#[derive(Clone, Copy)]
struct Seminfo {
    semmap : i32,
        ...
        if key != IPC_PRIVATE {
                                      ...
                                      .copied()
            {
                if flags & IPC_CREAT != 0 && flags & IPC_EXCL != 0 {
                    ...
                    match registry.by_id
                                  .get(&id)
                    {
                        Some(set) if nsems <= set.values.len() => {
                            let requested = flags & 0o666;
                            let read_ok =
                                requested & 0o444 == 0 || has_access(set, false, uid, gid);
                            let write_ok =
                                requested & 0o222 == 0 || has_access(set, true, uid, gid);
                            if read_ok && write_ok {
                                Ok(id)
                            } else {
                                Err(ErrNo::EACCES)
...
fn import_ops(pointer : usize, count : usize) -> Result<Vec<UserSemBuf>, ErrNo> {
    ...
    operations.try_reserve_exact(count)
              .map_err(|_| ErrNo::ENOMEM)?;
    ...
    loop {
        ...
        let outcome = {
            ...
            if let Some(blocked) = blocked {
                ...
                let set = registry.by_id
                                  .get_mut(&id)
                                  .expect("set checked above");
                match blocked {
                ...
                for operation in &operations {
                    if operation.flags & SEM_UNDO != 0 && operation.op != 0 {
                        let index = operation.num as usize;
                        undo_delta[index] = match undo_delta[index].checked_sub(operation.op as i32)
                        {
                ...
                for (number, delta) in undo_delta.iter()
                                                 .copied()
                                                 .enumerate()
                {
                    if delta == 0 {
                    ...
                    let current = registry.undo
                                          .get(&(task_id, id, number as u16))
                                          .copied()
                                          .unwrap_or(0);
                ...
                set.values
                   .copy_from_slice(&scratch[..value_count]);
                for operation in &operations {
        ...
        if deadline.as_ref()
                   .is_some_and(PollDeadline::expired)
        {
...
pub(crate) fn sys_semctl(args : SyscallArgs) -> UserRet {
    ...
    match command {
        IPC_INFO | SEM_INFO => {
            // stress-ng 等用 IPC_INFO/SEM_INFO 探测系统信号量限制。
            if argument == 0 {
        ...
        SETVAL => {
            ...
            set.ctime = timestamp;
            registry.undo
                    .retain(|(_, semid, number), _| *semid != id || *number as usize != semnum);
            UserRet::from_success(0)
        }
        GETALL => {
            if argument == 0 {
            ...
            let bytes = unsafe {
                core::slice::from_raw_parts(values.as_ptr()
                                                  .cast::<u8>(),
...
fn set_all(id : i32, pointer : usize, uid : u32, gid : u32) -> UserRet {
    ...
    let mut values = Vec::new();
    if values.try_reserve_exact(count)
             .is_err()
    {
...
pub(crate) fn task_exit(task_id : usize) {
    let pid = task::process_task_snapshot(task_id).map(|snapshot| {
                                                      snapshot.pid
                                                              .raw()
                                                              .min(i32::MAX as usize)
                                                      as i32
    ...
    for ((owner, semid, number), adjustment) in undo {
        if owner != task_id {
            registry.undo
                    .insert((owner, semid, number), adjustment);
            continue;
        }
        if let Some(set) = registry.by_id
                                   .get_mut(&semid)
        {
            if let Some(value) = set.values
