...
pub(crate) fn ensure_process_signal_state(pid: ProcessId) -> Result<(), ErrNo> {
    ...
    ipc::signal::with_registry(|registry| {
        ...
        for snapshot in descriptors.iter().skip(1) {
            if !registry.has_thread(snapshot.task_id) {
                let _ = registry.register_thread(leader.task_id, snapshot.task_id, snapshot.tid.raw());
            }
        }
    });
    Ok(())
...
pub(crate) fn timer_tick(interrupted_user: bool) {
    ...
    if let Ok(snapshot) = ensure_current_signal_state() {
        ...
        if let Ok(cpu_signals) =
            ipc::signal::with_registry(|registry| registry.account_cpu(snapshot.pid.raw(), user_delta, elapsed))
        {
...
pub(crate) fn deliver_pending_signal(
    ...
    if pending.action.flags & ipc::signal::SA_RESTART != 0 {
        if let Some((syscall_nr, args)) = restart {
            ActiveTrapFrame::prepare_syscall_restart(&mut original, syscall_nr, args.as_regs(), 4);
        }
    }
    ...
    context.prepare_signal_handler(
...
pub(crate) fn sys_rt_sigprocmask(args: SyscallArgs) -> UserRet {
    ...
    let old = match ipc::signal::with_registry(|registry| registry.update_mask(task_id, how, new_set)) {
        Ok(old) => old,
...
pub(crate) fn sys_rt_sigtimedwait(args: SyscallArgs) -> UserRet {
    ...
    let deadline = if timeout == 0 {
        ...
        let timeout = match copy_from_user_struct::<UserTimespec>(timeout) {
            Ok(timeout) if timeout.sec >= 0 && timeout.nsec >= 0 && timeout.nsec < 1_000_000_000 => {
                timeout
            }
    ...
    let sig = loop {
        ...
        let ticks = match deadline {
            Some(deadline) => {
                ...
                let tick_ns = (wateros_base_config::task::SCHED_TIMER_PERIOD_MS as u128) * 1_000_000;
        ...
        let _ = ipc::signal::with_registry(|registry| registry.begin_signal_wait(task_id, wait_set));
        let still_waiting = || {
            ipc::signal::with_registry(|registry| {
                registry
                    .pending(task_id)
...
pub(crate) fn sys_rt_sigaction(args: SyscallArgs) -> UserRet {
    ...
    if act != 0 {
        ...
        let action = ipc::signal::SignalAction {
