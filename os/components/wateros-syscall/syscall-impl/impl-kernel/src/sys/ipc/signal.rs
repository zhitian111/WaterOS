...
pub(crate) fn sys_rt_sigsuspend(args : SyscallArgs) -> UserRet {
    ...
    // EINTR 后仍保持临时 mask；take_deliverable() 会把原 mask 写入 signal
    // frame，并由 rt_sigreturn 恢复。提前恢复会令仅临时解除屏蔽的信号再次
    // 变成不可投递。
...
fn processes_in_group(pgid : ProcessId) -> Vec<ProcessId> {
    task::all_process_pids().into_iter()
                            .filter(|pid| task::process_pgid(*pid) == Some(pgid))
                            .collect()
}
...
fn resolve_kill_targets(pid : isize) -> Result<Vec<ProcessId>, ErrNo> {
    match pid {
        ...
        0 => {
            ...
            Ok(processes_in_group(current.pgid))
        }
        -1 => {
            let current = task::current_process_snapshot().ok_or(ErrNo::ESRCH)?;
        ...
        _ => {
            ...
            let targets = processes_in_group(ProcessId::from_raw(pgid));
            if targets.is_empty() {
                Err(ErrNo::ESRCH)
            } else {
                Ok(targets)
