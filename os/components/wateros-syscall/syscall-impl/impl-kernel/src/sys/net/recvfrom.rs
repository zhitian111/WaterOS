...
pub(crate) fn sys_recvfrom(args: SyscallArgs) -> UserRet {
    ...
    if crate::unix_sock::is_unix_fd(fd) {
        return match crate::unix_sock::recvfrom_unix(
            fd,
            buf_ptr,
    ...
    let lease = match receive_blocking(&socket, len.min(SYSCALL_IO_MAX), nonblocking) {
        Ok(Some(lease)) => lease,
    ...
    if kind == stack::SocketKind::Udp && progress.error.is_none() {
        let (ip, port) = lease.source();
        if let Err(error) = write_source_address(ip, port, addr_ptr, addrlen_ptr, source_capacity) {
    ...
    if flags & MSG_PEEK != 0 {
        let _ = lease.finish(0, false);
        return if progress.copied > 0 || progress.error.is_none() {
    ...
    let finish = lease.finish(progress.copied, progress.error.is_none());
    match finish {
        ...
        Ok(stack::SocketRecvFinish::Fault) => {
            UserRet::from_error(ErrNo::EFAULT)
        }
...
fn source_address_capacity(addr_ptr: usize, addrlen_ptr: usize) -> Result<Option<u32>, ErrNo> {
    ...
    let addrlen = crate::user_copy::copy_from_user_struct::<u32>(addrlen_ptr)
