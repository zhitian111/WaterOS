...
use crate::sys::time::timer::{account_child_cpu, child_cpu_from_exited, write_child_rusage, write_zero_rusage, ticks_to_timeval, ChildCpuTicks};
...
pub(crate) fn wake_clear_child_tid_for_task(task_id: task::TaskId) -> usize {
    ...
    if let Err(err) = clear_result {
        log::warn!(
            "[exit] clear_child_tid write failed task_id={} tid={} addr={:#x}: {:?}",
            task_id,
            tid_raw,
            addr,
            err,
...
fn user_siginfo_exited(pid: task::ProcessId, exit_code: isize) -> UserSigInfo {
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(
            &fields as *const SigchldFields as *const u8,
...
fn finish_waitid_process_result(
    parent_pid: task::ProcessId,
    pid: task::ProcessId,
    exited_tasks: Vec<task::ExitedTask>,
    siginfo_ptr: usize,
...
fn finish_waitid_stopped_result(
    pid: task::ProcessId,
    signo: u8,
    siginfo_ptr: usize,
    rusage_ptr: usize,
...
fn finish_waitid_continued_result(
    pid: task::ProcessId,
    siginfo_ptr: usize,
    rusage_ptr: usize,
    nowait: bool,
...
fn wait_target_from_pid(pid: isize, caller_pgid: task::ProcessId) -> Result<WaitTarget, ErrNo> {
    ...
    Ok(WaitTarget::Specific(task::ProcessId::from_raw(
        pid as usize,
    )))
}

fn find_exited_child_for_wait(
    parent_pid: task::ProcessId,
    target: WaitTarget,
...
fn find_stopped_child_for_wait(
    parent_pid: task::ProcessId,
    target: WaitTarget,
) -> Option<task::ProcessDescriptor> {
    match target {
        ...
        WaitTarget::Specific(child_pid) => task::stopped_child_ready_for_wait(parent_pid, child_pid),
    }
}

fn find_continued_child_for_wait(
    parent_pid: task::ProcessId,
    target: WaitTarget,
) -> Option<task::ProcessDescriptor> {
    match target {
...
fn wait_for_child_exit(
    ...
    loop {
        if let Some(child) = find_exited_child_for_wait(parent_pid, target) {
            ...
            return finish_wait_process_result(parent_pid, child.pid, exited, exit_code_ptr, rusage_ptr);
        }
    ...
    loop {
        if want_exited {
            if let Some(child) = find_exited_child_for_wait(parent_pid, target) {
                ...
                return finish_waitid_process_result(
                    parent_pid, child.pid, exited, siginfo_ptr, rusage_ptr,
                );
            }
        }
        if want_stopped {
            if let Some(child) = find_stopped_child_for_wait(parent_pid, target) {
                ...
                return finish_waitid_stopped_result(child.pid, signo, siginfo_ptr, rusage_ptr, nowait);
            }
        }
        if want_continued {
            if let Some(child) = find_continued_child_for_wait(parent_pid, target) {
                return finish_waitid_continued_result(child.pid, siginfo_ptr, rusage_ptr, nowait);
            }
        }
        if !has_waitable_child(parent_pid, target) {
            return UserRet::from_error(ErrNo::ECHILD);
        ...
        if wait_for_child_event(parent_pid, target, want_exited, want_stopped, want_continued)
...
fn wait_for_child_event(
    ...
    task::wait_on_while(wait_target, || {
        has_waitable_child(parent_pid, target)
            && !has_pending_wait_event(parent_pid, target, want_exited, want_stopped, want_continued)
    })
}
