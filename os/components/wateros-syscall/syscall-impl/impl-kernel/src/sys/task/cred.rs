...
pub(crate) fn sys_geteuid() -> UserRet {
    // TODO: 实现完整 capability 模型后移除硬编码 root
...
pub(crate) fn sys_getgroups(args : SyscallArgs) -> UserRet {
    ...
    for (i, g) in cred.supplementary_groups
                      .iter()
                      .take(ngroups)
                      .enumerate()
    {
    ...
    if copy_to_user(list_ptr,
                    &bytes[..write_len * core::mem::size_of::<u32>()]).is_err()
    {
        return UserRet::from_error(ErrNo::EFAULT);
    }
...
pub(crate) fn sys_setgroups(args : SyscallArgs) -> UserRet {
    ...
    for (i, group) in groups.iter_mut()
                            .take(ngroups)
                            .enumerate()
    {
