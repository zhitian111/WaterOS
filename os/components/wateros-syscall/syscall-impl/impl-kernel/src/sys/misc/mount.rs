...
pub(crate) fn sys_mount(args: SyscallArgs) -> UserRet {
    ...
    let mount_point = match resolve_path_at(crate::sys::fs::path_at::AT_FDCWD, target.as_str()) {
        Ok(p) => p,
        Err(e) => return UserRet::from_error(e),
    };
    ...
    if flags & MS_MOVE != 0 {
        ...
        let source = match resolve_path_at(crate::sys::fs::path_at::AT_FDCWD, source.as_str()) {
            Ok(p) => p,
            Err(e) => return UserRet::from_error(e),
        };
