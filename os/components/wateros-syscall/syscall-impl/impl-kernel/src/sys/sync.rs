...
fn sync_fd(op : &str, fd : usize) -> UserRet {
    log::info!("[iozone-probe][sys_{op}] begin fd={}", fd);
    match vfs::fd::with_current_io(fd, |handle| handle.flush()) {
        Ok(()) => {
            log::info!("[iozone-probe][sys_{op}] end fd={} ret=0", fd);
            UserRet::from_success(0)
        }
        Err(err) => {
            let errno = vfs_error_to_errno(err);
            log::info!("[iozone-probe][sys_{op}] end fd={} err={:?} errno={:?}",
                       fd,
                       err,
                       errno);
...
pub(crate) fn sys_sync(_args : SyscallArgs) -> UserRet {
    ...
    log::info!("[iozone-probe][sys_sync] begin flush_all_open_files");
    let _ = vfs::fd::flush_all_open_files();
    log::info!("[iozone-probe][sys_sync] end ret=0");
    UserRet::from_success(0)
}
