...
///
/// 先前使用 `read_time_tick() + TIMER_SLICE_TICKS` 的 **裸 `time` CSR 增量**；在部分 QEMU/频率
/// 组合下若与 SBI `set_timer` 期望的刻度或固件内部换算不一致，可能导致 **deadline 落在「现在」之前**
/// 或极近，从而 **S 态定时器中断连发**。此时 GDB `SIGINT` 常在 `trampoline_start`（`__alltraps`）
/// 处采样到 PC，看起来像「卡在 trampoline」，实为 **trap 风暴**而非 `sret` 损坏。
...
extern "C" fn wateros_kernel_trap_handler(frame : *mut u8) {
    ...
    match trap_cause {
        TrapCause::Exception(Exception::UserEnvCall) => {
            ...
            match task::current_task_snapshot() {
                ...
                None => {
                    info!(
                        "[syscall] task=<no snapshot> user_pc={:#x} user_sp={:#x} nr={} args=[{:#x},{:#x},{:#x},{:#x},{:#x},{:#x}]",
                        cx.user_pc(),
                        cx.user_sp(),
                        syscall_nr,
                        regs[0],
            ...
            if let Some(snap) = task::current_task_snapshot() {
                info!(
                    "[syscall] task_id={} nr={} ret={}",
                    snap.id,
                    syscall_nr,
                    syscall_ret
                );
            } else {
                info!("[syscall] task=<no snapshot> nr={} ret={}", syscall_nr, syscall_ret);
            }
        ...
        TrapCause::Exception(Exception::StorePageFault) => {
            // 先前仅 `debug!` 并继续 `sret`：若 fault 发生在用户态，会回到同一 sepc 立即再 fault，
            // 形成无限 trap 风暴；在 INFO 日志级别下毫无输出，表现为「sret 后卡死」。
            if cx.returns_to_user() {
                panic!(
                    "[trap] user memory fault {:?} sepc={:#x} stval={:#x} frame_scause={:#x} (fatal; was spinning in fault loop if only debug logged)",
                    trap_cause,
                    cx.user_pc(),
                    cx.fault_addr(),
                    raw_cause,
            ...
            debug!("[trap] kernel page fault: cause={:?} raw_cause={:#x?} pc={:#x?} fault_addr={:#x?}",
                   trap_cause,
                   raw_cause,
                   cx.user_pc(),
                   cx.fault_addr());
    ...
    // --- 返回路径（与 `trap.asm` 成对）：本函数返回后 **没有** 更多 Rust 代码会执行；汇编从 `sp` 指向的
    // `TrapContext` 装载 CSR/通用寄存器并 `sret`。因此不会出现「已成功回到用户态」的 INFO 日志——除非
    ...
    // 3. `restore_current_trap_frame(frame)`：把 TCB 内已更新的 `TrapContext` **拷回** `trap.asm` 传入的
    ...
    if cx.returns_to_user() && !restored {
        panic!(
            "restore_current_trap_frame failed before sret to user (current task trap_frame missing?)"
        );
    }
}
