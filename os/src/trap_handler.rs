...
fn kill_current_user_task(context: &str, trap_cause: TrapCause, cx: &TrapContext) -> ! {
    if let Some(snapshot) = task::current_task_snapshot() {
        warn!(
            "[trap] killing user task ({}) cause={:?} pc={:#x} fault_addr={:#x} task_id={} \
               parent_id={:?} state={:?}",
            context,
            trap_cause,
            cx.user_pc(),
        ...
        warn!(
            "[trap] killing user task ({}) cause={:?} pc={:#x} fault_addr={:#x} (no current \
               task snapshot)",
            context,
            trap_cause,
            cx.user_pc(),
...
fn fatal_kernel_trap(
    ...
    error!(
        "[trap] fatal kernel trap ({}) cause={:?} raw_cause={:#x} pc={:#x} fault_addr={:#x} \
            returns_to_user={}",
        context,
        trap_cause,
        raw_cause,
...
extern "C" fn wateros_kernel_trap_handler(frame: *mut u8) {
    ...
    match trap_cause {
        TrapCause::Exception(Exception::UserEnvCall) => {
            ...
            trace!(
                "[syscall] nr={} user_pc={:#x} user_sp={:#x} \
                    args=[{:#x},{:#x},{:#x},{:#x},{:#x},{:#x}]",
                syscall_nr,
                cx.user_pc(),
                cx.user_sp(),
    ...
    if cx.returns_to_user() {
        ...
        // 前也不会用该槽位预测下一次 trap。
        trace!(
            "[trap] sret to user pc={:#x} sp={:#x} address_space_token={:#x} \
    ...
    trace!(
        "[trap] restore_current_trap_frame restored={}",
        restored
    );
    if cx.returns_to_user() && !restored {
        panic!(
            "restore_current_trap_frame failed before sret to user (current task trap_frame \
                missing?)"
        );
    }
}
