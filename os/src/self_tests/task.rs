//! 任务调度、等待/超时、退出与回收、以及用户态（含可选根卷 ELF）相关的内核自检。
//!
//! **用途**：在 `kernel_main` 已初始化 MM 与 `task` 子系统后，通过多组内核任务与用户任务
//! 验证 `sleep`/`yield`、`block`/`wake`、等待队列超时、`wait_for_exit`/`reap`、以及
//! `spawn_user_task_spec` 与 trap 快照等契约。
//!
//! **入口**：[`spawn_all`] 由 `kernel_main` 调用；其中在启用 `impl-sv39` 时会先尝试加载
//! [`mm::kernel_mm::DEFAULT_USER_ELF_PATH`] 对应的真实用户 ELF 并派生观察任务，再创建
//! stage2/3/4 内核自检任务与内核映射的用户态自检任务，以降低自检对默认用户程序首轮调度的干扰。
//!
//! **后续替换点**：自检任务与用户镜像地址/系统调用号为 QEMU 调试路径上的固定约定，若
//! 用户 ABI 或镜像布局变更，应同步调整本模块常量与断言。
...
extern "C" fn stage3_exit_reaper_task(exit_target_task_id: usize) -> ! {
    ...
    let exited_task = task::reap_exited_task(exit_target_task_id).expect(
        "exit-reaper must observe zombie task before reaping",
    );
    assert_eq!(
...
extern "C" fn elf_user_observer_task(elf_task_id: usize) -> ! {
    trace!(
        "[elf-selftest] elf observer entered elf_task_id={}",
        elf_task_id
    );
    info!(
        "[elf-selftest] observer waiting for elf task id={}",
        elf_task_id
    );
    let _ = task::wait_for_task_exit_for_ticks(elf_task_id, 256);
    if let Some(exited) = task::reap_exited_task(elf_task_id) {
        info!(
            "[elf-selftest] reaped elf task exit_code={}",
            exited.exit_code
        );
    } else {
...
extern "C" fn stage4_user_observer_task(user_task_id: usize) -> ! {
    info!(
        "[task-stage4] observer task started, user_task_id={}",
        user_task_id
    );
    ...
    let exited_task = task::reap_exited_task(user_task_id)
        .expect("stage4 observer must reap exited user task");
    assert_eq!(
        exited_task.id, user_task_id,
    ...
    info!(
        "[task-stage4] observer reaped user task {} with exit_code={}",
        exited_task.id, exited_task.exit_code
    );
    task::exit_current(99);
}

/// 先启动根卷默认 ELF 用户任务，再启动 stage2/3/4 自检任务，降低自检对真实用户程序的调度干扰。
pub fn spawn_all() {
    #[cfg(feature = "impl-sv39")]
    {
        trace!(
            "[elf-selftest] try load ELF path={} (before scheduler self-tests)",
            mm::kernel_mm::DEFAULT_USER_ELF_PATH
        );
        match mm::kernel_mm::from_elf_path(mm::kernel_mm::DEFAULT_USER_ELF_PATH) {
            Ok(loaded) => {
                trace!(
                    "[elf-selftest] load ok entry={:#x} satp={:#x} image=[{:#x},+{:#x}) stack=({:#x},{:#x}]",
                    loaded.entry_pc,
                    loaded.satp,
                    loaded.image_base,
                    loaded.image_size,
                ...
                info!(
                    "[elf-selftest] spawned ELF user task id={} path={}",
                    elf_task_id,
                    mm::kernel_mm::DEFAULT_USER_ELF_PATH
                );
            }
            Err(err) => {
                warn!(
                    "[elf-selftest] skip ELF load ({:?}) path={}",
                    err,
                    mm::kernel_mm::DEFAULT_USER_ELF_PATH
                );
                trace!(
                    "[elf-selftest] skip detail: err={:?} — stage4 kernel-mapped user task still runs for MM self-test",
                    err
                );
            }
        }

        mm::kernel_mm::ensure_user_execute_for_kernel_va(
            stage4_user_task_entry as *const () as usize,
        );
        let stack_bottom = VirtAddr(STAGE4_USER_STACK_TOP - STAGE4_USER_STACK_SIZE);
        let stack_top = VirtAddr(STAGE4_USER_STACK_TOP);
        mm::kernel_mm::map_anon_range_user(
            stack_bottom,
            stack_top,
    ...
    trace!(
        "[task-stage4] spawn kernel-mapped user task entry={:#x} kernel_satp={:#x} image=[{:#x},+{:#x}) stack_top={:#x}",
        stage4_entry,
    ...
    trace!("[task-stage4] kernel-mapped user_task_id={}", user_task_id);
    let user_observer_task_id =
        task::spawn_kernel_task(stage4_user_observer_task, user_task_id);
    info!(
        "[task-stage2] spawned kernel tasks: blocked={}, sleep={}, waker={}",
        blocked_task_id, sleep_task_id, waker_task_id
    );
    info!(
        "[task-stage3b] spawned self-test tasks: timeout={}, exit_target={}, exit_waiter={}, exit_reaper={}, wait_queue_id={}",
    ...
    info!(
        "[task-stage4] spawned user self-test tasks: user={}, observer={}",
