...
impl BootPlan {
    ...
    fn parse(command_line: Option<&str>) -> Self {
        ...
        for field in command_line.unwrap_or("").split_ascii_whitespace() {
            ...
            match key {
                ...
                key if key.starts_with("wos.") => warn!("[{LOG_TAG}] ignored unknown boot option {key}"),
                _ => {}
            }
        }
        ...
        if plan.mode == OperatorMode::Run && plan.script.is_none() {
            plan.invalid = true;
        }
        if plan.invalid {
            ...
            plan.on_exit = ExitPolicy::Shell;
            plan.tty = TtyMode::Interactive;
            plan.log.get_or_insert(LevelFilter::Warn);
        }
        plan
    }
}
...
fn shell_candidates(requested: Option<&str>) -> Vec<ShellCandidate> {
    ...
    if let Some(path) = requested {
        let argv0 = if path.ends_with("busybox") { "sh" }
                    else if path.ends_with("bash") { "bash" }
                    else { "sh" };
        result.push(ShellCandidate { program: path.to_string(), argv0 });
    }
    for (program, argv0) in [("/bin/bash", "bash"),
                             ...
                             ("/musl/busybox", "sh")]
    {
        if !result.iter().any(|candidate| candidate.program == program) {
            result.push(ShellCandidate { program: program.to_string(), argv0 });
        }
    }
    result
}
...
fn shell_environment(shell: &str) -> Vec<String> {
    vec![
        "PATH=/root/.cargo/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/glibc".to_string(),
        "HOME=/root".to_string(),
        "USER=root".to_string(),
...
fn run_shell_once(requested: Option<&str>, script: Option<&str>) -> bool {
    for candidate in shell_candidates(requested) {
        ...
        info!("[{LOG_TAG}] trying shell={} argv={argv:?}", candidate.program);
        if let Some(code) = crate::user_bringup_common::run_one_elf_argv_env_exit(
            LOG_TAG,
            &candidate.program,
            &argv,
            &env_refs,
        ) {
            warn!("[{LOG_TAG}] shell={} exited code={code}", candidate.program);
            return true;
        }
    }
...
extern "C" fn console_input_main(_arg: usize) -> ! {
    loop {
        if let Some(event) = vfs::fd::poll_console_input_once() {
            let _ = syscall::send_kernel_signal_to_process_group(event.process_group,
                                                                  event.signal);
        } else {
...
/// Missing bootargs are recoverable on a serial console. The menu deliberately
/// offers only modes that do not need an additional path; arbitrary scripts
/// remain available through `wos.mode=run wos.script=/...`.
fn plan_from_command_line(command_line: Option<&str>) -> BootPlan {
    if command_line.is_some() {
        return BootPlan::parse(command_line);
    }

    tty::configure(tty::ConsoleTtyMode::Interactive);
    start_console_input_task();
    runtime::console::write_str(
        "\r\nWaterOS bootargs missing. Select within 3 seconds:\r\n\
         [1] automatic tests (default)\r\n[2] operator rescue shell\r\n> ",
    );
    let deadline = task::current_tick().saturating_add(
        3_000u64.div_ceil(base_config::task::SCHED_TIMER_PERIOD_MS.max(1)),
    );
    while task::current_tick() < deadline {
        match tty::prepare_read(32) {
            tty::TtyPreparedRead::Data(read) => {
                let shell = read.bytes().iter().copied().find(|byte| !byte.is_ascii_whitespace()) ==
                            Some(b'2');
                let len = read.bytes().len();
                let _ = tty::finish_read(read, len, true);
                return if shell {
                    BootPlan::parse(Some("wos.mode=shell"))
                } else {
                    BootPlan::defaults()
                };
            }
            tty::TtyPreparedRead::Eof => break,
            tty::TtyPreparedRead::Pending => {
                let _ = task::sleep_for_ticks(1);
            }
        }
    }
    runtime::console::write_str("\r\nWaterOS: menu timeout, continuing automatic tests.\r\n");
    BootPlan::defaults()
}
...
extern "C" fn operator_main(_arg: usize) -> ! {
    ...
    match plan.mode {
        ...
        OperatorMode::Run => {
            let _ = run_shell_once(plan.shell.as_deref(), plan.script.as_deref());
        }
        OperatorMode::Shell => {
            let _ = run_shell_once(plan.shell.as_deref(), None);
        }
    }
    ...
    loop {
        if !run_shell_once(plan.shell.as_deref(), None) {
...
mod tests {
    ...
    fn defaults_to_automatic_mode() {
        assert_eq!(BootPlan::parse(None).mode, OperatorMode::Auto);
    }
    ...
    fn requested_shell_precedes_stable_fallbacks_without_duplicates() {
        ...
        let paths: Vec<&str> = candidates.iter().map(|candidate| candidate.program.as_str()).collect();
        assert_eq!(paths,
                   ["/musl/busybox", "/bin/bash", "/bin/sh", "/glibc/busybox"]);
    }
}
