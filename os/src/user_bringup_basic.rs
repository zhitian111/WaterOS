...
fn warn_missing_basic_assets() {
    ...
    for path in [
        ...
        match view.exists(path) {
            Ok(true) => info!(
                "[basic-bringup] rootfs asset present: {}",
                path
            ),
            Ok(false) => warn!(
                "[basic-bringup] rootfs asset MISSING: {} (oscomp fstat/openat \
                                may fail)",
                path
            ),
            Err(e) => warn!(
                "[basic-bringup] rootfs check {}: {:?}",
                path, e
            ),
        }
    }
...
pub fn run_stage_03() {
    ...
    #[cfg(any(feature = "impl-sv39", feature = "impl-loongarch64"))]
    {
        ...
        info!(
            "[basic-bringup] will try {} ELF(s) under /glibc/basic/ and /musl/basic/ \
               (brk/mmap/munmap via stage-02-mm)",
            n
        );
        info!(
            "[basic-bringup] spawn only enqueues user tasks; CPU-side user code runs after \
               task::run_first_task()"
        );
        #[cfg(feature = "vfs-bridge")]
        warn_missing_basic_assets();
        for path in BASIC_GLIBC_PATHS
            .iter()
            .chain(BASIC_MUSL_PATHS)
        {
            match mm::kernel_mm::from_elf_path(path) {
                Ok(loaded) => {
                    info!(
                        "[basic-bringup] loaded path={} entry={:#x} image=[{:#x},+{:#x}) \
                    ...
                    info!(
                        "[basic-bringup] spawned user task {} for {}",
                        tid, path
                    );
                }
                Err(e) => {
                    warn!(
                        "[basic-bringup] skip path={}: {:?}",
                        path, e
                    );
                }
            }
