...
const SCRIPT_PATHS : &[&str] = &["/glibc/basic_testcode.sh",
...
const GLIBC_BASIC_INLINE : &str = "echo \"#### OS COMP TEST GROUP START basic ####\"; cd ./basic; \
                                   ...
                                   wait waitpid write yield; do echo \"Testing $i :\"; ./$i; \
                                   done; cd ..; echo \"#### OS COMP TEST GROUP END basic ####\"";
...
fn run_script_or_inline(script_path : &str) {
    if script_path == GLIBC_BASIC_SCRIPT {
        ...
        let argv = [GLIBC_BUSYBOX_PATH,
                    "sh",
                    "-c",
                    GLIBC_BASIC_INLINE];
        info!("[{LOG_TAG}] SCRIPT INLINE path={script_path} argv={argv:?}");
        let _ =
            crate::user_bringup_common::run_one_busybox_argv(LOG_TAG, GLIBC_BUSYBOX_PATH, &argv);
        return;
    }
...
fn run_busybox_probes() -> bool {
    let echo_argv = [GLIBC_BUSYBOX_PATH,
                     "echo",
                     "__busybox_echo_ok__"];
    ...
    let sh_c_argv = [GLIBC_BUSYBOX_PATH,
                     "sh",
                     "-c",
                     "echo __busybox_sh_c_ok__"];
