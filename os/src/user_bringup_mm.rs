...
//! 加载 MM 相关测程 ELF 并 `spawn` 用户任务（与
//! `os/tem/glibc/basic`、`os/tem/musl/basic` 等镜像布局一致；缺失文件则 `warn`
//! 跳过）。
//!
//! 须在 `fs::init` 之后调用。**`spawn` 只把用户任务放入就绪队列**；此时尚未发生
//! `__switch`， 测程的 `ecall` 等要到 `kernel_main` 末尾
//! `task::run_first_task()` 切入多任务后才会在 CPU 上执行。
...
pub fn run_stage_02() {
    ...
    #[cfg(any(feature = "impl-sv39", feature = "impl-loongarch64"))]
    {
        ...
        info!(
            "[mm-bringup] spawn only enqueues user tasks; CPU-side user code runs after \
               task::run_first_task()"
        );
        for path in MM_GLIBC_PATHS
            .iter()
            .chain(MM_MUSL_PATHS)
        {
            match mm::kernel_mm::from_elf_path(path) {
                Ok(loaded) => {
                    ...
                    info!(
                        "[mm-bringup] spawned user task {} for {}",
                        tid, path
                    );
                }
                Err(e) => {
                    warn!(
                        "[mm-bringup] skip path={}: {:?}",
                        path, e
                    );
                }
            }
